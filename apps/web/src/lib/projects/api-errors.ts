...
export function projectJsonError(code: ProjectApiErrorCode, message: string, status: number) {
  return NextResponse.json({ success: false, errorCode: code, message }, { status })
}
...
export function isProjectCanvasSchemaMissing(error: unknown): boolean {
  ...
  return /(CanvasWorkflow|CanvasNode|CanvasEdge|CanvasComment|ProjectMember|ProjectRole|Project|Asset|lastOpenedAt|workflowId|dataUrl|metadataJson)/.test(text)
