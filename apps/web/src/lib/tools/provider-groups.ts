...
export function getCanvasProviderNotice(provider: ToolProvider | null) {
  if (!provider) return ''
  return getCanvasProviderNoticeFromStatus(provider.status)
}

export function getCanvasProviderNoticeFromStatus(status: ToolProviderStatus | null | undefined): string {
  if (!status || status === 'available') return ''
  if (status === 'mock') return '模拟生成，不会调用第三方 API'
  if (status === 'bridge-only') return '仅生成桥接请求，不会真实调用'
  if (status === 'not-configured') return '未配置 API key、endpoint 或 adapter'
  if (status === 'coming-soon') return '即将接入，当前不可真实调用'
  return ''
}
