...
export function normalizeRecoveryFailureCode(code: string) {
  ...
  if (code === 'PROVIDER_RETRIEVE_NOT_AVAILABLE' || code === 'provider_retrieve_not_available' || code === 'provider_retrieve_not_implemented' || code === 'unrecoverable_provider_retrieve_not_implemented') return 'no_recovery_source'
...
export function terminalRecoveryAction(code: string, attemptedUrls: string[] = []): MediaRecoveryAction {
  ...
  if (normalized === 'provider_invalid_parameter') return 'provider_invalid_parameter'
  if (normalized === 'provider_env_missing') return 'provider_env_missing'
