...
test('resizes the task editor with eight handles while retaining fixed dialog rails', () => {
  ...
  assert.match(canvasModuleSource, /\.canvas-task-editor-resize-handle\.is-n\) \{ top: 0; left: 18px; width: calc\(100% - 36px\); height: 16px; cursor: ns-resize; \}/)
  assert.match(canvasModuleSource, /\.canvas-task-editor-resize-handle\.is-e\) \{ top: 18px; right: 0; width: 16px; height: calc\(100% - 36px\); cursor: ew-resize; \}/)
  assert.match(canvasModuleSource, /\.canvas-node-resize-handle\) \{[\s\S]*?background: transparent;[\s\S]*?box-shadow: none;/)
  assert.match(canvasModuleSource, /\.canvas-node-resize-handle\.is-nw\) \{ top: 0; left: 0; cursor: nwse-resize; \}/)
  assert.match(canvasModuleSource, /\.canvas-node-resize-handle\.is-n\) \{ top: 0; left: 18px; width: calc\(100% - 36px\); height: 16px; cursor: ns-resize; \}/)
  assert.match(canvasModuleSource, /\.canvas-node-resize-handle\.is-e\) \{ top: 18px; right: 0; width: 16px; height: calc\(100% - 36px\); cursor: ew-resize; \}/)
