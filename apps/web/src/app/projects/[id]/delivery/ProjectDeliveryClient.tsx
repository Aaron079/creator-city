...
export function ProjectDeliveryClient({ projectId, projectTitle, initialShare, assets }: Props) {
  ...
  const shareUrl = initialShare ? `/delivery/${initialShare.token}` : ''
  const includedAssetIds = useMemo(() => new Set(initialShare?.items?.map((item) => item.assetId).filter(Boolean)), [initialShare?.items])
  ...
  async function addAsset(asset: DeliveryAssetForClient) {
    ...
    if (!res.ok) {
      setMessage({ ok: false, text: data.message ?? '添加素材失败' })
      return
    }
    ...
    <div className="space-y-6">
      <section className="rounded-lg border border-white/10 bg-white/[0.03] p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          >
            {initialShare ? '刷新交付链接' : '创建交付链接'}
          </button>
        </div>
        {initialShare ? (
          <div className="mt-4 rounded-md border border-white/10 bg-black/20 p-3">
            <div className="break-all font-mono text-xs text-cyan-100">{shareUrl}</div>
            <a href={`/delivery/${initialShare.token}`} target="_blank" className="mt-3 inline-flex text-sm font-semibold text-cyan-200 underline">
              打开客户交付页
            </a>
          </div>
        ) : null}
      ...
      <section className="grid gap-6 lg:grid-cols-[1fr_1fr]">
        <div className="rounded-lg border border-white/10 bg-white/[0.03] p-5">
          <h2 className="text-base font-semibold text-white">可交付素材</h2>
          {assets.length === 0 ? (
            <p className="mt-4 text-sm text-white/45">空项目，请先生成文本或图片资产。</p>
          ) : (
              ...
              {assets.map((asset) => (
                <div key={asset.id} className="flex items-center justify-between gap-3 rounded-md border border-white/10 bg-white/[0.03] p-3">
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 text-xs text-white/40">{asset.type}</div>
                  </div>
                  <button
                    type="button"
                    onClick={() => void addAsset(asset)}
                    disabled={!initialShare || includedAssetIds.has(asset.id) || pendingAssetId === asset.id}
        ...
        <div className="rounded-lg border border-white/10 bg-white/[0.03] p-5">
          ...
          {!initialShare?.comments?.length ? (
            <p className="mt-4 text-sm text-white/45">暂无客户反馈。</p>
          ) : (
            <div className="mt-4 space-y-3">
              {initialShare.comments.map((comment) => (
