...
import { CityEntrySection } from '@/components/home/CityEntrySection'
...
const DIRECTORS: DirectorDef[] = [
  { id:'commercial', icon:'🔥', name:'商业导演', personality:'快节奏，强冲突', bestFor:'院线大片', strengths:['节奏压缩','爆点前置'], scores:{pacing:98,conflict:92,emotion:72,clarity:75}, review:async(s,t)=>{for(const x of['压缩节奏…','前置冲突…']){t(x);await new Promise(r=>setTimeout(r,480))} return [{...s[0],description:s[0].description+' 镜头极速切换。'},{title:'⚡冲突前置',description:s[2].description+' 肾上腺素飙升。'},{...s[1],description:s[1].description+' 音效拉满。'},{title:'🔥强力收尾',description:s[3].description+' 震撼全场。'}]} },
  { id:'auteur',     icon:'🌫️',name:'作者导演', personality:'情绪优先，意象表达', bestFor:'影展',   strengths:['长镜头','情绪留白'],   scores:{pacing:50,conflict:60,emotion:97,clarity:58}, review:async(s,t)=>{for(const x of['感受情绪…','注入隐喻…']){t(x);await new Promise(r=>setTimeout(r,480))} return [{...s[0],description:s[0].description+' 长镜头凝视。'},{title:'🌫️记忆碎片',description:s[1].description+' 画面过曝。'},{...s[2],description:s[2].description+' 雨声替代对白。'},{title:'∞开放结局',description:s[3].description+' 黑幕七秒。'}]} },
  { id:'control',    icon:'📐', name:'控制型导演',personality:'结构精准，逻辑严密', bestFor:'悬疑片', strengths:['精准剪辑','逻辑链'],   scores:{pacing:82,conflict:78,emotion:70,clarity:96}, review:async(s,t)=>{for(const x of['梳理逻辑…','布局信息…']){t(x);await new Promise(r=>setTimeout(r,480))} return [{title:'📐建立规则',description:s[0].description+' 每处埋下伏笔。'},{...s[1],description:s[1].description+' 信息精确释放。'},{title:'🔑关键反转',description:s[2].description+' 伏笔引爆。'},{...s[3],description:s[3].description+' 线索收束。'}]} },
]
...
function CreateModal({onClose,onCreate}:{onClose:()=>void;onCreate:(a:WriterDef)=>void}){
  ...
  const submit=()=>{if(!name.trim())return;onCreate({id:`u_${Date.now()}`,icon:'🧑‍🎨',name:name.trim(),creator:'我',personality:mani||style,bestFor:uc||'通用',strengths:picked.length?picked:['自定义'],scores:S_SC[style],draft:mkD(mani,style)});onClose()}
  return <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={e=>{if(e.target===e.currentTarget)onClose()}}>
    <div className="city-card border border-city-accent/30 w-full max-w-md space-y-3">
      ...
      <div><p className="text-xs text-gray-500 mb-2">风格</p><div className="flex gap-2">{[['explosive','⚡爆款'],['emotional','🎨情绪'],['structural','📐结构']].map(([v,l])=><button key={v} onClick={()=>setStyle(v)} className={`flex-1 py-1.5 rounded-lg text-xs border transition-all ${style===v?'bg-city-accent/20 border-city-accent/50 text-city-accent-glow':'border-city-border text-gray-400'}`}>{l}</button>)}</div></div>
...
function DistPanel({db,hist,onPick}:{db:Record<string,Stats>;hist:string[];onPick:(w:string,d:string)=>void}){
  ...
  return <div className="city-card border border-city-border space-y-3"><div className="flex items-center justify-between"><h3 className="text-sm font-semibold">⚡ 分发推荐</h3><span className="text-xs text-gray-600">60% 爆款 · 20% 个性 · 20% 探索</span></div>
    {TABS.map(t=><div key={t.l}><p className="text-xs text-gray-500 mb-1.5">{t.l}</p><div className="flex flex-wrap gap-1.5">{[...new Set(t.ks)].map(k=>{const[wId,dId]=k.split('+');const w=WRITERS.find(x=>x.id===wId);const d=DIRECTORS.find(x=>x.id===dId);if(!w||!d)return null;const lb=lbl(k);return<button key={k} onClick={()=>onPick(wId,dId)} className={`flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs border transition-all hover:opacity-80 ${qualityScore(k,db)<60?'opacity-40':''} ${t.c}`}>{w.icon}{w.name}<span className="opacity-40 mx-0.5">+</span>{d.icon}{d.name}<span className={`ml-1 text-xs px-1 rounded border ${lb.c}`}>{lb.l}</span></button>})}</div></div>)}
  </div>}
...
function AICreatorBox(){
  const [input,setInput]=useState('');const [phase,setPhase]=useState<Phase>('idle');const [thought,setThought]=useState('');const [scenes,setScenes]=useState<Scene[]>([])
  const [writerId,setWriterId]=useState('classic');const [dirId,setDirId]=useState('commercial');const [userAgents,setUserAgents]=useState<WriterDef[]>([])
  ...
  const rec:Rec|null=input.trim()&&phase==='idle'?REC_MAP[detectTheme(input)]:null
  const curLocked=PRO_COMBOS.has(key)&&!unlocked.has(key)

  async function handleRun(){
    if(!input.trim()||phase!=='idle'||curLocked)return;setScenes([]);setFeedback(null);setHist(h=>[...new Set([...h,key])])
    setDb(prev=>upd(prev,key,'uses'));setPhase('writing');setThought('构思场景结构…');await new Promise(r=>setTimeout(r,900))
    const draft=writer.draft(input.trim());setPhase('directing');setThought('')
    const final=await director.review(draft,setThought);setThought('');setScenes(final);setPhase('done')
  }
  ...
  return <section className="py-16 px-4"><div className="max-w-2xl mx-auto space-y-3">
    ...
    <div className="flex items-center justify-between"><p className="text-xs text-gray-600">AI 创作工作台</p><button onClick={()=>setShowDash(true)} className="text-xs px-2.5 py-1 rounded-lg bg-city-accent/10 text-city-accent-glow border border-city-accent/20 hover:bg-city-accent/20 transition-colors">📊 创作者后台</button></div>
    ...
    <div className="city-card border border-city-accent/20 space-y-4">
      ...
      <div className="flex gap-2 border-t border-city-border pt-3">
        <input type="text" value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleRun()} placeholder="告诉团队你的故事创意……" className="flex-1 bg-city-bg border border-city-border rounded-xl px-4 py-3 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-city-accent/60 transition-colors"/>
        <button onClick={handleRun} disabled={!input.trim()||phase!=='idle'||curLocked} className="px-5 py-3 rounded-xl bg-city-accent hover:bg-city-accent-glow disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-semibold transition-all flex-shrink-0">{curLocked?'🔒 已锁定':phase==='idle'?'开始创作':'创作中…'}</button>
      </div>
      ...
      {thought&&phase==='writing'   &&<div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-city-accent/5 border border-city-accent/15"><span>{writer.icon}</span><span className="w-1.5 h-1.5 rounded-full bg-city-accent animate-pulse"/><span className="text-xs text-city-accent-glow">{writer.name} · {thought}</span></div>}
      {thought&&phase==='directing' &&<div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-city-accent/5 border border-city-accent/15"><span>{director.icon}</span><span className="w-1.5 h-1.5 rounded-full bg-city-accent animate-pulse"/><span className="text-xs text-city-accent-glow">{director.name} · {thought}</span></div>}
    </div>
    {scenes.length>0&&<div className="city-card border border-city-border space-y-3">
      <div className="flex items-center gap-2 flex-wrap"><div><p className="text-lg font-bold text-city-accent-glow">{vs} <span className="text-xs text-gray-500 font-normal">爆款指数</span></p><p className="text-xs text-gray-500">已超过 {beatPct}% 的创作</p></div>{vs>=85&&<span className="text-xs px-2 py-0.5 rounded-full bg-city-gold/20 text-city-gold border border-city-gold/30">🔥 爆款</span>}
        <div className="ml-auto flex items-center gap-2">{feedback?<span className="text-xs text-gray-500">{feedback==='good'?'👍 感谢':'👎 已记录'}</span>:<><span className="text-xs text-gray-600">推荐准确？</span><button onClick={()=>{setFeedback('good');setDb(prev=>upd(prev,key,'likes'))}} className="text-sm hover:scale-110 transition-transform">👍</button><button onClick={()=>{setFeedback('bad');setDb(prev=>upd(prev,key,'dislikes'))}} className="text-sm hover:scale-110 transition-transform">👎</button></>}<button onClick={()=>{setPhase('idle');setScenes([]);setFeedback(null)}} className="text-xs text-gray-600 hover:text-white transition-colors ml-1">重置</button></div>
      </div>
      {scenes.map((s,i)=><div key={i} className="flex gap-3 rounded-xl border border-city-border bg-city-surface/60 p-3 hover:border-city-accent/30 transition-all"><div className="w-6 h-6 flex-shrink-0 rounded-md bg-city-accent/10 border border-city-accent/20 flex items-center justify-center text-city-accent-glow text-xs font-bold">{i+1}</div><div><p className="text-xs font-semibold text-white mb-0.5">{s.title}</p><p className="text-xs text-gray-400 leading-relaxed">{s.description}</p></div></div>)}
...
// ─── Creator Leaderboard (static mock) ───────────────────────────────────────

const CREATOR_LB=[{n:'Ada',ic:'🎨',ag:5,rev:'¥3,210',badge:'text-city-gold'},{n:'Luna',ic:'🎵',ag:3,rev:'¥2,840',badge:'text-gray-300'},{n:'Max',ic:'🎬',ag:2,rev:'¥1,680',badge:'text-city-rose'}]

export default function HomePage(){
  return <main className="relative min-h-screen overflow-hidden">
    <div className="absolute inset-0 bg-city-grid opacity-30 pointer-events-none" aria-hidden="true"/>
    <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-gradient-radial from-city-accent/20 to-transparent rounded-full blur-3xl pointer-events-none" aria-hidden="true"/>
    <div className="relative z-10"><HeroSection/><AICreatorBox/><CityEntrySection/><HotWorksSection/><NewsfeedSection/>
      <section className="py-10 px-4 border-t border-city-border/30"><div className="max-w-2xl mx-auto"><div className="flex items-center justify-between mb-3"><h2 className="text-sm font-bold text-city-accent-glow">🏅 创作者榜</h2><span className="text-xs text-gray-600">本月收益</span></div>
        <div className="grid grid-cols-3 gap-3">{CREATOR_LB.map((c,i)=><div key={c.n} className="city-card border border-city-border p-3 flex items-center gap-3"><span className={`text-sm font-bold w-4 flex-shrink-0 ${c.badge}`}>{i+1}</span><div className="min-w-0"><p className="text-xs font-semibold">{c.ic} {c.n}</p><p className="text-xs text-gray-500">{c.ag} Agent</p><p className="text-xs text-city-emerald font-semibold">{c.rev}</p></div></div>)}</div>
      </div></section>
      <section className="py-24 text-center border-t border-city-border/50"><h2 className="text-3xl font-bold mb-4">准备好建造你的创作帝国了吗？</h2><p className="text-gray-400 mb-8 max-w-md mx-auto">加入已在 Creator City 创作的数千名创作者。</p><Link href="/auth/register" className="inline-flex items-center gap-2 px-8 py-4 bg-city-accent hover:bg-city-accent-glow text-white font-semibold rounded-xl transition-all duration-200 glow">进入 Creator City</Link></section>
    </div></main>
}
