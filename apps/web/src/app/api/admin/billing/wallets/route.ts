...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await fetch(`${getApiBase()}/api/v1/credits/admin/wallets`, {
      headers: { Authorization: `Bearer ${token}` },
      cache: 'no-store',
    })
    if (!res.ok) return NextResponse.json({ error: 'Failed to fetch wallets' }, { status: res.status })
    return NextResponse.json({ wallets: await readJson<unknown>(res) })
  } catch (err) {
