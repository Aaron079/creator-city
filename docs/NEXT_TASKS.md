...
| P1-4C | BYOK-first generation UX polish | DONE | P1 | IMPLEMENTATION | /account/providers, canvas provider selector, generate error messages, no-key states | P1-4B membership QA | type-check PASS · lint PASS · build PASS | P1-4C-1 commit ee96b33: default billingMode=user_provider_account; INSUFFICIENT_CREDITS→BYOK guidance; /account/providers copy aligned; selector reordered |
