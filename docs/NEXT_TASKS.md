...
| P1-4D-3 | Inquiry UI (buyer + seller) | TODO | P1 | IMPLEMENTATION | /assets/[id] buyer inquiry submit + status; /assets/[id] seller respond/reject/close; /account/marketplace-inquiries buyer/seller list page | P1-4D-2 | Browser acceptance: buyer can submit, seller can respond | No payment UI; only contact intent flow |
| P1-4D-5 | Marketplace inquiry QA | TODO | P0 | QA_ONLY | POST inquiry → membership gate; seller respond/reject/close; buyer close; idempotent; no MarketplaceOrder/LicenseGrant created | P1-4D-3 UI done | All 17 acceptance criteria PASS | Manual test with non-member, member, seller, ADMIN sessions |
