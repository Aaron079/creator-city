...
- A primary-button double-click on blank Canvas opens that same `创建节点` picker. It never directly creates a Video node or opens a task dialog.
...
- Native double-click is not used for secondary buttons because browsers do not consistently emit it there. The Canvas tracks secondary-click timing directly, while primary-button double-click reuses the existing picker.
...
- Add focused source/interaction boundary tests for the 320ms two-click threshold, Canvas picker opening, primary-button double-click picker opening without direct Video creation, node downstream picker opening, and the absence of the legacy single-right-click card menu path.
