...
Last updated: 2026-06-21 (P1-CANVAS-TOOL-PANEL-MIGRATION-PHASE-3 IMPLEMENTED / WAITING_BROWSER_QA — commit 04b6563; DirectorToolPanelFrame extended with bodyClassName + clearLabel optional props (additive, Phase 1/2 panels unaffected); CharacterBiblePanel fully migrated: 👤 角色圣经 / CHARACTER BIBLE; indigo accent; clearLabel=新建角色; 2-column body with bodyClassName="min-h-0 flex-1 overflow-hidden"; self-contained backdrop z-1199; open prop removed (VCW conditionally renders); SceneBiblePanel non-embedded path migrated: 🎬 场景圣经 / SCENE BIBLE; amber accent; clearLabel=新建场景; embedded path in CreativeAssetsPanel untouched; ShotListBuilderPanel migrated: 🎞 分镜表 / SHOT LIST; violet accent; self-contained backdrop z-1199 (VCW backdrop div removed); summary bar shows shot count + total duration; clearLabel=复制分镜清单; handleCreate removed (生成全部镜头 covers create+generate); type-check PASS · lint PASS · pushed)
