...
Last updated: 2026-08-13 (P0-CREATOR-INTELLIGENCE-SCRIPT-TO-STORYBOARD-ADVISORY-B VALIDATED / LOCAL_COMMITTED / WAITING_PUSH_APPROVAL -- commits `6985ba3`, `de75d22`, `7612f7a`, `caa7367`, `4c1ed49`, and `ef62dd2` add four deterministic local-only Storyboard Director advisory families: narrative purpose, composition hierarchy, continuity confirmation, and lighting motivation. Recipe V3 preserves only reviewed/ignored decisions under strict validation; V1/V2 Recipes upgrade in-memory without a database migration. The existing evidence inspector now shows local rule IDs and `ckr1_` receipts and supports review, ignore, restore, and precise linked-shot focus. Local advisories never block materialization; stale state still takes priority in shot markers. Full type-check, lint (existing warnings only), build, agent check, diff-check, 147 focused contracts, 3 AST boundary tests, and rendered browser component QA passed. The browser QA is a local bundled harness, so it does not claim Preview or Production persistence. No crawler, external model, Provider/BYOK, generation, billing/payment/credit, schema, env, cn-executor, or Production DB behavior changed. Push and Vercel deployment require explicit session confirmation.)
