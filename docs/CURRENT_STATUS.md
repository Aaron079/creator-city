...
Last updated: 2026-07-15 (P0-CANVAS-COMMERCIAL-MATURITY-GOLDEN-PATH-QA FAILED / BLOCKED_BY_LOCAL_DRAFT_OVERWRITE_RISK — authenticated Production Google Chrome QA on creator-city-vert.vercel.app; PRODUCT_BUG/P0: project center confirmed `Golden Path QA 2026-07-15` had 2 cloud nodes, but re-entering the same project automatically rendered a stale 1-node local draft and offered `保存到云端`, creating a direct path to overwrite the newer cloud canvas; Codex did not click save, so the 2-node server version was not overwritten; a second clean QA project verified native file-picker upload PASS, `/api/assets/upload` product flow created READY image asset `creator-city-logo`, Prompt + reference context persisted after a successful retry and restored after reload, and `/assets` found the private project-owned asset; first canvas save attempt failed closed with `云端保存失败，重试`, one retry succeeded, proving no false success but leaving a transient save reliability signal; PRODUCT_BUG/P1: reload after acknowledged save again displayed `保存到云端` despite restored content, and `/projects` plus Dashboard recent-project cards reported 0 assets while `/assets` and Dashboard aggregate reported 1; NodeToolCenter PASS for camera/lens/lighting/prompt/style/variant entries; result-dependent fullscreen/annotation/storyboard split correctly remained unavailable because a reference input is not a node result, so annotation/split end-to-end remains blocked rather than fabricated; `/tasks` showed 0 and remained read-only; authenticated page transitions sampled at 1.2–2.6s without hangs; second browser redirected to login with no project disclosure and is classified AUTH_BLOCKER for cross-browser restoration; BYOK page showed no connected API account; product Console warnings/errors/uncaught/hydration/maximum-depth/500/503 count=0; exact request counts are QA_HARNESS_LIMITATION, while no generate button, Provider call, billing/credits/wallet/payment/recharge action was initiated; no product code, API, schema, migration, env, package, lockfile, next.config, cn-executor, Provider, billing/payment, or Production DB change; next blocker task: P0-CANVAS-LOCAL-DRAFT-RECOVERY-VERSION-GUARD)
