...
Last updated: 2026-08-03 (P0-LAYOUT-3-LEFT-TOOL-RAIL PRODUCTION_DEPLOYED / WAITING_POST_DEPLOY_BROWSER_QA -- authenticated Production read-only inspection confirmed the 80px left rail sits at x=20, its add menu occupies x=124–434, and the nearest node begins at x=585, so the rail does not overlap the canvas. Add, Director, and Prompt menus each open rightward; console warning/error capture was empty. The inspection also found that independently tracked menus could remain mounted together. `dfe466a` replaces that split state with one exclusive Canvas dock menu, uses an explicit parent add-menu setter, and removes the now-dead active-tool state. TDD recorded the missing exclusive-menu reducer as RED, then passed 1/1; `pnpm --filter web type-check`, `pnpm lint` (existing warnings only), `pnpm build`, and `git diff --check` passed. Vercel Production deployment for `dfe466a` is Ready. A fresh Production browser check of the new bundle remains required and is not claimed here. No save, upload, generation, Provider, payment, billing, schema, environment, cn-executor, or Production DB action occurred.)
