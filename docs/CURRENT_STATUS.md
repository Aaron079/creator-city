...
Last valid commit: `201c795` (Canvas Smart Tools Tool 4 — Character Lock Basic — CLOSED / validated)
...
| Character Lock（角色一致性锁定基础版 · Tool 4） | 左侧 CanvasToolDock | ✅ validated |
| Version Compare / A-B Compare（版本对比面板） | — | next recommended |
...
14. **Canvas Smart Tools Tool 5 — Version Compare / A-B Compare Panel（版本对比面板）**
    - 已有资产对比工具：选择两个 image 节点，并排展示两版生成结果
    - 纯前端展示，不消耗 credits，不新增 API
    - 优先级：P1（下一步推荐，适合在角色锁定后继续工具链扩展）
    - Real Batch Variants / 批量真实生成：later（涉及 credits 并发失败处理，不直接做）
