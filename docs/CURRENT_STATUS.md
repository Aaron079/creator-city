...
| Canvas Smart Tools Tool 3A — Asset Variant Planner（资产变体规划器 · 左侧工具导航 · 变体方向卡片） | 🚧 IMPLEMENTED / browser validation pending | `3819d1c` |
...
| Asset Variant Planner（资产变体规划器 · Tool 3A） | — | next recommended |
| Character Lock / Batch Variants | — | not implemented |
...
11. **Canvas Smart Tools Tool 3A — Asset Variant Planner（资产变体规划器）— NEXT RECOMMENDED**
    - 安全"规划版"：用户选定种子节点后生成变体规划清单，不直接批量消耗 credits
    - 约束：不改 generate routes / billing / schema；每次生成仍需用户手动触发
    - 优先级：P1（紧接 Toolbar Cleanup 之后）

12. **错误提示产品化（P2）**
    - 去除剩余 `errorCode:`/`provider_*:` 前缀（OSS/media 类还有残留）

13. **NEXT_PUBLIC_API_URL / billing webhook（P2，单独排期）**
   - 确认 CN 部署是否启用支付链路
