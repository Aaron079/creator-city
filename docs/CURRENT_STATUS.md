...
Last updated: 2026-08-13 (P0-CREATOR-INTELLIGENCE-SCRIPT-TO-STORYBOARD-ADVISORY-B VALIDATED / LOCAL_COMMITTED / WAITING_PUSH_APPROVAL -- commits `6985ba3`, `de75d22`, `7612f7a`, `caa7367`, `4c1ed49`, `ef62dd2`, `772fe19`, and `46e59a7` add four deterministic local-only Storyboard Director advisory families: narrative purpose, composition hierarchy, continuity confirmation, and lighting motivation. Recipe V3 preserves only reviewed/ignored decisions under strict validation; V1/V2 Recipes upgrade in-memory without a database migration, including existing fresh sketch boards. Advisory review, ignore, and restore never invalidate a current sketch board. The existing evidence inspector shows local rule IDs, `ckr1_` receipts, and real source-line evidence, with review, ignore, restore, and precise linked-shot focus. Local advisories never block materialization; stale state still takes priority in shot markers. Full type-check, lint (existing warnings only), build, agent check, diff-check, 151 focused contracts, AST boundary tests, and rendered local browser component QA passed. The browser QA is a local bundled harness, so it does not claim Preview or Production persistence. No crawler, external model, Provider/BYOK, generation, billing/payment/credit, schema, env, cn-executor, or Production DB behavior changed. Push and Vercel deployment require explicit session confirmation.)
