...
Last updated: 2026-06-20 (P0-LEGACY-CREATE-V2-ROUTE-REMOVAL IMPLEMENTED / WAITING_BROWSER_QA — /create is the sole production canvas; /create-v2 now server-side redirects to /create (preserves ?projectId if present); independent legacy XYFlow canvas removed: deleted components/create-v2/ (6 files: CanvasV2Workspace/ProjectActions/AssetLibrary/Inspector/Node/Toolbar) + lib/canvas-v2/canvasV2Adapter.ts; TopNavigation dead code removed (handleCanvasV2Click + isV2 check); docs updated; QA/auth tools updated to /create; type-check PASS · lint PASS · build PASS · pushed)
