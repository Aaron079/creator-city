...
Last valid commit: Web3 Foundation P1-1 IMPLEMENTED — RefundRequest + Manual Review Intent（49ea001）（migration 20260612000003_add_marketplace_refund_request，MarketplaceRefundRequest 模型 + MarketplaceRefundRequestStatus 枚举 PENDING/APPROVED/REJECTED/CANCELLED，buyer POST/PATCH/GET /api/me/marketplace-orders/[id]/refund-request，admin GET /api/admin/marketplace/refund-requests + PATCH approve/reject，seller 订单列表含退款状态 badge，buyer COMPLETED 订单完整退款申请 UI + 撤销 + adminNote 展示，支付确认对话框 5 条免责声明，无账务变动/无 LicenseGrant revoke）
