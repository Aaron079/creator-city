...
Last valid commit: Web3 Foundation P1-0 IMPLEMENTED — Marketplace Settlement v1（migration 20260612000002_add_marketplace_settlement_v1，COMPLETED enum value + completedAt column，CreditLedgerType MARKETPLACE_PURCHASE + MARKETPLACE_SELLER_CREDIT，settle.ts 原子事务 buyer 扣款 + seller 加款 + LicenseGrant upsert，PATCH action:pay，buyer QUOTED 状态展示含余额+二次确认对话框，/marketplace COMPLETED 状态 ✓ 已获得授权，QUOTED 状态"去支付"链接，/assets/[id] seller 订单列表含 COMPLETED + 到账积分展示）
