...
Last updated: 2026-08-02 (P0-CREDITS-WALLET-CREATION-RACE-RECOVERY PRODUCTION_DEPLOYED / PREVIEW_QA_BLOCKED -- commit `8691fc8` makes the existing wallet bootstrap recover a concurrent unique-key collision by rereading the wallet created by the competing request. Balance, ledger, payment, Provider, generation, schema, environment, and Production DB semantics are unchanged. TDD recorded the missing collision-recovery helper as RED, then the focused regression passed 1/1; `pnpm --filter web type-check`, `pnpm lint` (existing warnings only), `pnpm type-check`, `pnpm build`, and `git diff --check` passed. Vercel Production deployment at `8691fc8` is Ready. The Git-backed isolated Preview deployment is reachable, but a disposable registration reports its configured database schema as uninitialized, so authenticated first-wallet browser QA is correctly `PREVIEW_CONFIG_BLOCKED`, not PASS. Do not use Production to create a test account or alter its database.)
