...
Last valid commit: `3819d1c` (Canvas Smart Tools Tool 3A — Asset Variant Planner — browser validated)
...
12. **P0-small：Media Preview Fallback for Expired Historical Assets（独立任务）**
    - 历史节点 OSS signed URL 过期 → CanvasNodeCard 通过 /api/media/proxy 返回 502，console 有大量噪音
    - 修复方向：proxy 返回 502/404 时 CanvasNodeCard 降级到占位图（无破坏性，纯 UI）
    - 约束：不改 cn-executor / generate routes / billing / schema
    - 优先级：P0-small（消除 console 噪音，改善开发/调试体验）
