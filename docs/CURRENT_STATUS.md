...
Last updated: 2026-06-21 (P1-CANVAS-TOOL-PANEL-MIGRATION-PHASE-3 IMPLEMENTED / WAITING_CLOSEOUT_QA — Shot List create-without-generate restored; DirectorToolPanelFrame extended with secondaryLabel/onSecondary/secondaryDisabled optional props + accent.secondary style (additive, all other panels unaffected); ShotListBuilderPanel: handleCreate restored (safe, no Provider call, no credits); footer now has 创建分镜节点 (N) as primary + 生成所选镜头 (N) as secondary; 生成所选镜头 requires explicit confirmation banner (shows count + Provider cost warning + 确认生成/取消); Character Bible frame validated · Scene Bible frame validated · Shot List frame validated · create-without-generate workflow restored · generation remains explicit and user-confirmed · no automatic Provider calls; type-check PASS · lint PASS · build PASS)
