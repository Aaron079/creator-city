...
Last valid commit: Web3 Foundation P0-6 IMPLEMENTED — MarketplaceOrder PENDING-only（migration 20260612000000_add_marketplace_order，MarketplaceOrderStatus enum PENDING/CANCELLED/REJECTED，MarketplaceOrder model，POST /api/marketplace/listings/[id]/orders 创建 PENDING 申请，GET seller 看待处理申请，GET /api/me/marketplace-orders buyer/seller 订单列表，/marketplace 付费 listing 申请付费授权按钮，/assets/[id] seller 看待处理申请数 + buyer 看申请状态，/api/me/stats 加 orderSubmitted/Received count，不扣款不创建 paid LicenseGrant 不接 billing）
