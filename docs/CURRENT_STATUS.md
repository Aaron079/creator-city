...
Last updated: 2026-09-07 (P1-CANVAS-ANCHORED-NODE-CONTEXT-SURFACES IMPLEMENTED / LOCAL_VALIDATED / PREVIEW_AUTH_QA_PENDING -- selected Text, Image, and Video nodes now use one fixed visual column: display node, a 48px `任务 / 工具 / 资产` navigation surface 18px beneath it, then a 210px compact context surface 6px below. The old toolbar-owned popups and duplicate bottom-dock task launcher are removed. Tool entries state truthful outcomes (task adjustment, derived image, structured text, preview, or unavailable reason) without triggering Provider, billing, or generation behavior. The Canvas pans upward when lower viewport space is insufficient; that automatic layout pan is excluded from Canvas persistence. Focused contracts passed 30/30; type-check, lint (existing warnings only), production build, `agent:check`, and diff-check passed. The safe Preview Playwright case is skipped because this environment has no injectable authenticated fixture, so authenticated visual/layout interaction remains QA_HARNESS_LIMITATION rather than a claimed Preview pass. No API route, Provider/BYOK, generation dispatch, billing/payment/credit/wallet, schema/migration, env, package/lockfile, cn-executor, or Production DB behavior changed.)
