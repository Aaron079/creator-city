...
Last updated: 2026-07-14 (P0-PROD-CONTINUITY-PREVIEW-BOOTSTRAP-W PRODUCTION_BACKEND_VALIDATED / WAITING_CHROME_BROWSER_QA — implementation commit ab857fa promoted to main; Vercel Production deployment dpl_3KvorpLuL39ex2SFipHXJdzjgPp4 Ready and creator-city-vert.vercel.app aliases active; Functions moved from iad1 to hnd1 beside the Tokyo Supabase pooler; WRXZ DATABASE_URL remained write-only sensitive throughout and was promoted inside Vercel without reading, printing, or saving its value; prior DATABASE_URL remains encrypted and isolated to Preview branch recovery/schema-bootstrap-normalization, while immutable deployment dpl_75pJzmdP4kGkkZL3B4stsCMbzv8u is the instant rollback point; Production API/database canary PASS: register 201, credentials login 200, auth session 200, project create 201, canvas initial load/save/reload 200 with test node restored, assets list 200, /create 200, /assets 200; manual recharge and China payment create both fail closed with 503 PLATFORM_CREDITS_RECHARGE_DISABLED; Provider requests=0, generation POSTs=0; deployment error logs over the validation window=0, P2024=0, DB-related errors=0; pre-deploy validation PASS: Tokyo region static test 1/1, type-check, lint (pre-existing warnings only), build, diff-check; Chrome extension/install/native-host checks PASS but Codex Chrome control bootstrap fails with `Cannot redefine property: process`, so authenticated visual/browser-console QA is not claimed; no schema, migration, generate route, Provider adapter, billing, payment code, cn-executor, package, lockfile, env file, or Production DB migration changes)
