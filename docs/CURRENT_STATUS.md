...
Last valid commit: `b1831dd` (Canvas Tool Dock Grouping — Director / Asset / Character sub-nav — IMPLEMENTED / browser validation pending)
...
| Canvas Tool Dock Grouping（导演/资产/角色 分组子导航重构） | ✅ IMPLEMENTED / browser validation pending | `b1831dd` |
...
| 镜头词典（Camera Lexicon） | 左侧 CanvasToolDock | ✅ validated |
| 资产变体规划器（Asset Variant Planner · Tool 3A） | 左侧 CanvasToolDock | ✅ validated |
| Media Preview Fallback（历史预览过期 fallback） | CanvasNodeCard.tsx | ✅ validated |
| Character Lock（角色一致性锁定基础版 · Tool 4） | Character 分组 | ✅ validated |
| Canvas Tool Dock Grouping（Director/Asset/Character 分组子导航） | CanvasToolDock.tsx | ✅ IMPLEMENTED / browser validation pending |
| A/B Compare（版本对比面板） | — | future · Asset 分组 |
| Shot Doctor（镜头诊断） | — | future · Director 分组 |
| Character Consistency Checker | — | future · Character 分组 |
...
14. ~~**Canvas Tool Dock Grouping（导演/资产/角色 分组子导航重构）** — ✅ IMPLEMENTED (commit `b1831dd`, 2026-06-06)~~
...
15. **Canvas Smart Tools Tool 5 — A/B Compare Panel（Asset 分组下）**
...
## Canvas Tool Dock Grouping — IMPLEMENTED / browser validation pending

**Commit:** `b1831dd` | **Date:** 2026-06-06  
...
| 未来工具 | 归属组 | 位置 |
|---|---|---|
| Shot Doctor | Director | items 追加 |
| Motion Director | Director | items 追加 |
| A/B Compare | Asset | items 追加 |
| Keyframe Extractor | Asset | items 追加 |
| Consistency Checker | Character | items 追加 |
| Multi-character Binding | Character | items 追加 |

### Validation Checklist (19 criteria)

| # | 验收项 | 状态 |
|---|---|---|
| 1 | 左侧工具栏只有3个分组图标：Clapperboard / Layers / UserRound | pending |
| 2 | 不再有独立的一级 Camera Lexicon / Variant Planner / Character Lock 图标 | pending |
| 3 | 点击 Clapperboard → 弹出 Director 子导航，含"镜头词典"条目 | pending |
| 4 | 点击 Layers → 弹出 Asset 子导航，含"变体规划器"条目 | pending |
| 5 | 点击 UserRound → 弹出 Character 子导航，含"角色锁定"条目 | pending |
| 6 | 子导航面板 Framer Motion 动画进入（opacity+x） | pending |
| 7 | 再次点击同一组图标 → 子导航关闭 | pending |
| 8 | 点击子导航中工具条目 → 对应面板打开，子导航关闭 | pending |
| 9 | 面板已打开时，对应条目显示"已打开"badge（蓝色） | pending |
| 10 | 一级分组图标在面板打开时高亮（is-active） | pending |
| 11 | 子导航底部显示未来工具预告文本（不可点击） | pending |
| 12 | 打开 Director 子导航时，Add 菜单自动关闭 | pending |
| 13 | 镜头词典/变体规划器/角色锁定面板功能保持不变 | pending |
| 14 | 面板互斥逻辑保持（由 workspace 管理，与 Task 4 一致） | pending |
| 15 | User 菜单正常（打开子导航时 user 菜单关闭） | pending |
| 16 | Add 节点菜单正常（+ 按钮）| pending |
| 17 | Stop All Generations 按钮逻辑不变 | pending |
| 18 | Settings 菜单不变 | pending |
| 19 | 图片/视频/文字生成链路无回归 | pending |
