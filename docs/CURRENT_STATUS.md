...
Last updated: 2026-09-08 (P1-CANVAS-TASK-DIALOG-SINGLE-COLUMN-COMPACT-RAILS PREVIEW_DEPLOYED / PREVIEW_SMOKE_PASS / AUTH_QA_PENDING -- branch `codex/video-gate-qa` is pushed at `ff8e7d8`. Vercel Preview deployment `dpl_jV7pr6s2Rfc15FRpUmfaU4fZZoGL` is Ready at `https://creator-city-4lzzqvc3w-aarons-projects-e26427fd.vercel.app`, and the branch alias is `https://creator-city-git-codex-video-gate-qa-aarons-projects-e26427fd.vercel.app`. Deployment logs confirm it built commit `ff8e7d8`; root returned 200 and `/create` returned the expected 307 login redirect. Headless Chromium recorded zero product console errors and zero API 5xx; two Chromium GPU/WebGL ReadPixels performance messages are tooling-only warnings. Authenticated Preview Task interaction remains pending because this run has no injected signed-in fixture. The deployed UI change is the verified single-column compact Task surface: 52px reference rail, fixed header, scrollable body, fixed compact footer, and 52px billing rail. No API, Provider/BYOK, generation, billing/payment/credit/wallet, persistence, schema/migration, env, package/lockfile, cn-executor, Production, or Production DB behavior changed.)

Last updated: 2026-09-08 (P1-CANVAS-TASK-DIALOG-SINGLE-COLUMN-COMPACT-RAILS LOCAL_VALIDATED / PREVIEW_DEPLOYMENT_PENDING -- commits `71adb5d`, `9ecfab7`, `58e38af`, `db83321`, and `e086f89` correct the selected-node Task dialog's compact layout without changing task behavior. The old two-column compact grid is replaced by one vertical surface: a full-width 52px reference/context rail, a fixed prompt header, one independently scrollable prompt body, a compact fixed model/parameter/credit/generate footer, and a full-width 52px account/billing rail. Compact controls remain horizontally reachable inside their rails; they no longer occupy a separate right-hand column or create a blank half-dialog. Stable sizing now adds the top and bottom rails because both are vertically stacked. Chromium coverage passed all 42 focused layout/browser contracts across uploading/done image references, script application, image-to-video, and BYOK-missing-endpoint states. The constrained 390x300 video case measured 52px top controls / 42px header / 36px scrollable body / 48px footer / 52px bottom controls. `pnpm type-check`, lint (existing warnings only), `pnpm --filter web build`, `pnpm agent:check`, and `git diff --check` passed. Preview deployment and authenticated Preview interaction are not yet claimed. No API, Provider/BYOK semantics, generation dispatch, billing/payment/credit/wallet semantics, persistence, schema/migration, env, package/lockfile, cn-executor, Production, or Production DB behavior changed.)

Last updated: 2026-09-08 (P1-CANVAS-TASK-DIALOG-FIXED-SURFACES PREVIEW_DEPLOYED / LOCAL_VALIDATED / PREVIEW_AUTH_QA_PENDING -- commits `cbc64db` through `b0cf60c` refine the existing selected-node Task surface without changing task behavior. The compact `任务 / 工具 / 资产` navigation, selected node, and Task dialog now settle as one ordered vertical stack: supported stages keep all three within 16px stage margins, while physically constrained stages keep the dialog usable and allow the upper stack to extend offscreen without overlap. Stage-bound and zoom-aware pan deduplication reapplies the layout adjustment after a resize while excluding auto-pan itself from the key to prevent adjustment loops. The dialog uses a fixed mode/close header, one independently scrollable prompt body, and a fixed model/parameter/credit/generate footer. Existing upstream inputs, local image/text references, BYOK account details, status/error messages, upload/remove/apply actions, and missing-Endpoint guidance remain available; constrained stages switch them into horizontally reachable 70px control lanes. Stable noncompact measurement caching prevents ResizeObserver compact/expanded oscillation. Real Chromium coverage passed the complete stack at supported 390x800 and constrained 390x300 sizes, plus 390x300 image uploading/done, text ScriptCard application, real image-to-video mode, and selected BYOK missing-Endpoint states; the video case measured fixed top 70px, fixed bottom 70px, header 42px, scrollable prompt body 36px, and footer 82px within the 232px dialog. Focused layout/browser contracts passed 45/45 and adjacent task-input/save-scheduling regressions passed 43/43. `pnpm type-check`, `pnpm lint` (existing warnings only), `pnpm --filter web build`, `pnpm agent:check`, `git diff --check`, forbidden-zone audit, specification review, and code-quality review passed. Branch `codex/video-gate-qa` is pushed at `2333094`; Vercel Preview deployment `dpl_GN2y7HfCVXgX66h4rTJF4hnse3nA` is Ready at `https://creator-city-njlhknq3x-aarons-projects-e26427fd.vercel.app` and aliases the branch URL. Preview root returned HTTP 200, unauthenticated `/create` returned the expected 307 login redirect, and headless Chromium reached the login surface with no console errors, warnings, or API 5xx; the QA screenshot was visually inspected and then removed with the temporary Playwright output. Authenticated Preview Task interaction is not claimed because no injectable authenticated fixture is available and the existing Chrome control bridge disconnected, classified as QA_HARNESS_LIMITATION / AUTH_BLOCKER. No API route, Provider/BYOK semantics, generation dispatch, billing/payment/credit/wallet semantics, Canvas persistence, schema/migration, env, package/lockfile, cn-executor, Production DB, or Production change occurred.)

Last updated: 2026-09-07 (P1-CANVAS-ANCHORED-NODE-CONTEXT-SURFACES PREVIEW_DEPLOYED / LOCAL_VALIDATED / PREVIEW_AUTH_QA_PENDING -- selected Text, Image, and Video nodes now use one fixed visual column: display node, a 48px `任务 / 工具 / 资产` navigation surface 18px beneath it, then a 210px compact context surface 6px below. The old toolbar-owned popups and duplicate bottom-dock task launcher are removed. Tool entries state truthful outcomes (task adjustment, derived image, structured text, preview, or unavailable reason) without triggering Provider, billing, or generation behavior. The Canvas pans upward when lower viewport space is insufficient; that automatic layout pan is excluded from Canvas persistence. Focused contracts passed 30/30; type-check, lint (existing warnings only), production build, `agent:check`, and diff-check passed. `codex/video-gate-qa` was pushed at `6b43a5b`; the corresponding Preview `https://creator-city-g45yu5k7h-aarons-projects-e26427fd.vercel.app` returned HTTP 200. The safe Preview Playwright case is skipped because this environment has no injectable authenticated fixture, so authenticated visual/layout interaction remains QA_HARNESS_LIMITATION rather than a claimed Preview pass. No API route, Provider/BYOK, generation dispatch, billing/payment/credit/wallet, schema/migration, env, package/lockfile, cn-executor, or Production DB behavior changed.)

Last updated: 2026-09-07 (P1-CANVAS-NODE-TOOL-MENU-REGRESSION PREVIEW_VALIDATED / CLOSED -- commit `5d00104` keeps the selected-node floating toolbar inside the visible viewport rather than allowing a right-edge node to place its controls off-screen. The toolbar recomputes after viewport changes. Focused toolbar, NodeToolCenter, and Prompt Booster contracts passed 15/15; type-check, lint (existing warnings only), build, `agent:check`, and diff-check passed. Vercel Preview for `codex/video-gate-qa` is Ready at `5d00104`. Authenticated Chrome restored the isolated three-node QA canvas, selected the former right-edge independent text node, and opened its `工具` menu by normal mouse click. The full menu showed Prompt Booster, Script Segmentation, Narrative Beat Analysis, Shot List Builder, and Storyboard Director. No Generate, Provider/BYOK, payment, Production, API, database, schema, env, package, or cn-executor action occurred.)

Last updated: 2026-09-07 (P0-CANVAS-TOOL-PLUGIN-REGISTRY-PHASE-3A PREVIEW_QA_PASS / P1_TOOL_MENU_REGRESSION_OPEN -- commits `2688c74` through `536de17` strengthen the existing Prompt Booster instead of replacing it. Explicit suggestion selection is typed, validated, and isolated by locked `projectId + nodeId`; valid current suggestion IDs restore with canonical titles, stale selections stay inert, storage failures fail safely, and explicit clear/dismiss actions persist only to the locked target. The existing derived-node workflow copies the Prompt Booster selection only to the child after node creation while preserving the source Prompt and source state. Prompt Booster remains outside automatic generation prompt composition. Focused state, rendered panel, workspace, and registry contracts passed 51/51; type-check, lint (existing warnings only), build, `agent:check`, diff-check, forbidden-zone audit, per-task specification/code-quality reviews, and the final cross-phase review passed with no findings. `codex/video-gate-qa` is pushed at `f8f3c98`; Vercel Preview is Ready. Authenticated Chrome created an isolated Preview QA project with a Prompt-bearing source, independent text node, and Prompt Booster derived child. Source selection restored after close/reopen and cloud-save refresh; the derived child showed its Prompt Booster badge and enhanced Prompt; the source Prompt stayed unchanged; console warning/error count remained zero. No Generate, Provider/BYOK, payment, or production action was triggered. P1_PRODUCT_UI_ERROR: after selecting the independent Prompt-bearing text node, its visible `工具` button reproducibly did not render NodeToolCenter, while the source node's identical control did; this prevents an additional direct browser isolation interaction, though focused tests cover state isolation. No API route, Provider/BYOK, generation dispatch, billing/payment/credit/wallet, schema/migration, env, package/lockfile, cn-executor, or Production DB behavior changed.)

Last updated: 2026-09-04 (P0-CANVAS-TOOL-PLUGIN-REGISTRY-PHASE-2 VALIDATED / PRODUCTION_DEPLOYED -- commits `f0c0412` through `f92881f` place the existing Camera Control and Lighting & Atmosphere node-state reads, writes, and derived-node copies behind `tool-plugin-state.ts`. Existing localStorage keys and first-load legacy migration remain unchanged for normal panel use; derived copies now use non-migrating source reads, so a legacy-only source never receives a new node key as a side effect. Focused Canvas state/registry/boundary/save/recovery contracts passed 56/56; type-check, lint (existing warnings only), build, `agent:check`, and diff-check passed. Vercel Production deployed `f92881f` with Ready status. Authenticated Chrome QA restored the Golden Path local draft, loaded the project, displayed distinct Camera states (`sony-venice-2` and `red-v-raptor-8k`) plus Lighting state (`Backlight`), and reported no new product console error/warning without clicking Generate or Save. Browser request enumeration remains a QA_HARNESS_LIMITATION. No API route, Provider/BYOK, generation dispatch, billing/payment/credit, schema, env, cn-executor, or Production DB behavior changed.)
Last updated: 2026-09-03 (P1-TOOL-PLUGIN-REGISTRY PHASE_1 VALIDATED / PRODUCTION_DEPLOYED -- commits `f448a35`, `fcf26cb`, `48e2ec9`, and `53a1823` introduce a typed, additive Canvas tool-plugin registry for the existing Camera Control and Lighting & Atmosphere prompt contributions. The resolver enforces supported node kinds, preserves Camera-before-Lighting ordering, and normalizes non-media nodes to no contribution; the existing prompt helpers remain the canonical formatters, so Image/Video legacy output is byte-for-byte compatible. Focused registry, boundary, incremental-save, draft-recovery, and save-scheduling suites passed 45/45; type-check, lint (existing warnings only), build, `agent:check`, and diff-check passed. Vercel automatically deployed `53a1823` to Production and reported Ready. Authenticated Chrome QA restored the protected 106-node local draft in the isolated Golden Path project, displayed existing Camera and Lighting derived drafts, and returned to “已同步到云端” without clicking Generate. No product console warning/error was observed; the sole console error originated in the Codex browser extension. The project contained no Video node, so video-node panel persistence is a QA_HARNESS_LIMITATION, and network request enumeration was unavailable through the browser bridge. No Provider/BYOK, generation, billing/payment/credit, schema, env, cn-executor, or Production DB behavior changed.)

Last updated: 2026-08-24 (P1-CANVAS-INCREMENTAL-SAVE-PERFORMANCE VALIDATED / PRODUCTION_DEPLOYED -- commits `b8e7fec`, `c82b3a9`, `59dfc08`, and `6aa6984` replace per-row Canvas node/edge persistence with parameterized PostgreSQL bulk upserts and send only dirty entities for normal cloud saves. Existing full snapshots, workflow-version reservation, deletion acknowledgement, local-draft recovery, and failure semantics remain compatible. Focused unit/static contracts passed 14/14 and 13/13; full type-check, lint (existing warnings only), build, agent-check, and diff-check passed. A disposable local PostgreSQL 16 smoke inserted a node, conflict-updated it while retaining its stored image URL, and inserted an edge. Vercel Production deployment `https://creator-city-9o09hoaj5-aarons-projects-e26427fd.vercel.app` is Ready for `6aa6984` and aliases `https://creator-city-vert.vercel.app`. Authenticated Chrome QA used the isolated Golden Path project at 100 nodes: one safe Text node was added, cloud save acknowledged, refresh restored all 101 nodes, and console error/warning counts were zero. Exact Production request-payload/count capture remains a QA_HARNESS_LIMITATION; no Provider/generation/payment/billing/credit/wallet action, schema/env/package/cn-executor, or direct Production DB change occurred.)

Last updated: 2026-08-24 (P0-PROD-FRESH-NEON-PRODUCTION-CUTOVER VALIDATED / PRODUCTION_DATABASE_MIGRATED / AUTH_DB_CANARY_PASS -- Vercel Production `DATABASE_URL` was replaced with the verified Fresh Neon connection and restricted to the Production target. Prisma `migrate status` found all 17 migrations, `migrate deploy` reported no pending migrations, and the final status remained up to date. Production deployment `dpl_2Q3m4HkY9DwWZRNsL4YscTSyQVFS` is Ready and aliased to `https://creator-city-vert.vercel.app`. A no-account login canary returned the expected `401 INVALID_CREDENTIALS`, rather than a database-unavailable error, proving the deployed authentication read path reaches the new database. No `db push`, destructive SQL, data deletion, code, payment, credits, wallet, billing, Provider/BYOK, generate route, cn-executor, package, or old WRXZ project change occurred. This is a fresh database recovery: historic WRXZ accounts, projects, and assets are not restored by schema migration.)

Last updated: 2026-08-24 (P0-PRISMA-FRESH-DATABASE-BOOTSTRAP VALIDATED / PRODUCTION_CODE_DEPLOYED -- `main` merge commit `b49bb81` is Vercel Production Ready. Read-only Production smoke returned the Creator City home page and redirected unauthenticated `/create` to login with no browser console errors. The forward-only migration code is deployed, but no Production database migration, `db push`, destructive SQL, Production environment change, payment, credits, wallet, billing, Provider/BYOK, generate route, cn-executor, or package change was performed in this deployment.)

Last updated: 2026-08-24 (P0-PRISMA-FRESH-DATABASE-BOOTSTRAP VALIDATED / PREVIEW_DATABASE_BOOTSTRAPPED / PREVIEW_PERSISTENCE_SMOKE_PASS -- commits `f78b6d8`, `35708a1`, `24381a2`, and `cd93305` add one forward-only Prisma normalization migration for Canvas, Delivery, and Provider objects that were absent after replaying the historical migration chain on a blank PostgreSQL database. TDD first reproduced the exact absent-object set on a disposable local PostgreSQL 16 container, then the regression ran `prisma migrate deploy`, asserted `migrate status` is up to date, compared current Prisma models/enums against PostgreSQL catalogs, and reran deployment with no pending migrations. Static guard, Prisma generate, full type-check, lint (existing warnings only), build, agent check, and diff-check passed. A second confirmed-empty disposable Preview database received all 17 historical migrations with `migrate status` up to date and a second deploy reporting no pending migrations. Its sensitive `DATABASE_URL` is bound only to Preview branch `codex/prisma-fresh-database-bootstrap`; no shared or Production variable was modified. Preview registration, project creation, local image upload, save, and independent authenticated-context recovery now pass in the isolated safe-write suite. The initial storage failure was traced to an OSS endpoint configuration: a plain HTTP endpoint timed out from Vercel, then a bucket-qualified HTTPS endpoint duplicated the bucket hostname. The branch-only Preview `ALIYUN_OSS_ENDPOINT` override now uses the region root HTTPS endpoint; no key, bucket, shared, or Production variable changed. Current Preview `https://creator-city-ob1kxlqnl-aarons-projects-e26427fd.vercel.app` passed both persistent source-asset import and independent restore checks. No Production database migration, Production environment/configuration, payment, credits, wallet, billing, Provider/BYOK, generate route, cn-executor, package, or application env behavior changed.)

Last updated: 2026-08-17 (P0-CANVAS-FOUR-ZONE-SHELL-PHASE-1 LOCAL_VALIDATED / CLOSED -- commits `f30cb23` through `35ac79e` activate the selected-node Inspector inside the approved four-zone Canvas shell without changing tool semantics. The Inspector is a 320–420px desktop aside and becomes a keyboard-contained, 16px viewport-bounded narrow-screen sheet; task dialogs use the same 16px limits across all four Canvas stage edges. Commits `c1d80ca` and `35ac79e` bind dialog placement and sizing to the live Canvas stage, then remeasure it with ResizeObserver after Inspector or Dock changes. Existing node-scoped Camera, Lighting, and Prompt controls are exposed as node-identity-preserving Inspector shortcuts, while video remains click-to-load. Focus handoff and overlay gating keep the Inspector below active Canvas dialogs. Local automated validation passed: 6 static request-boundary contracts, 23 rendered layout/Inspector/Storyboard contracts at 1280x720 and 390x844, full type-check, lint (existing warnings only), build, and diff-check. No API, Provider/BYOK, generation, billing, schema, persistence, env, cn-executor, or Production DB semantics changed. Authenticated Preview safe-write and Production browser QA are not claimed: both remain AUTH_BLOCKER / ENV_BLOCKER pending historical Supabase recovery.)

Last updated: 2026-08-13 (P0-CREATOR-INTELLIGENCE-STORYBOARD-ADVISORY-EVALUATION VALIDATED / PRODUCTION_DEPLOYED / PRODUCTION_READ_ONLY_SMOKE_PASS -- commits `c2b8d09`, `d774485`, `44b534f`, `46799b5`, and `a3e786a` add an owned, offline evaluation matrix for the existing local Storyboard Director advisory evaluator. Eight deterministic synthetic Recipe V3 cases yield exactly 8 expected and emitted content findings, 8 exact matches, 2 required silent cases, and 1 scoped `ignored` decision invalidation after source identity changes. The gate double-evaluates every case, verifies advisory severity, stable receipt and finding identities, local rule IDs, and approved evidence ownership. The local-only AST guard now also scans the evaluation fixture and contract test. 156 focused contracts, 4 AST boundary tests, full type-check, lint (existing warnings only), build, agent check, and diff-check passed. `main` is pushed at `98d16ff`; Production root returned 200 with that SHA in the deployment response and unauthenticated `/create` redirected to login. This is a read-only deployment smoke, not authenticated advisory interaction. No UI, user data, network, crawler, external model, Provider/BYOK, generation, billing/payment/credit, schema, env, cn-executor, or Production DB behavior changed.)

Last updated: 2026-08-13 (P0-CREATOR-INTELLIGENCE-SCRIPT-TO-STORYBOARD-ADVISORY-B VALIDATED / PRODUCTION_DEPLOYED / PRODUCTION_NAVIGATION_SMOKE_PASS -- commits `6985ba3`, `de75d22`, `7612f7a`, `caa7367`, `4c1ed49`, `ef62dd2`, `772fe19`, and `46e59a7` add four deterministic local-only Storyboard Director advisory families: narrative purpose, composition hierarchy, continuity confirmation, and lighting motivation. Recipe V3 preserves only reviewed/ignored decisions under strict validation; V1/V2 Recipes upgrade in-memory without a database migration, including existing fresh sketch boards. Advisory review, ignore, and restore never invalidate a current sketch board. The existing evidence inspector shows local rule IDs, `ckr1_` receipts, and real source-line evidence, with review, ignore, restore, and precise linked-shot focus. Local advisories never block materialization; stale state still takes priority in shot markers. Full type-check, lint (existing warnings only), build, agent check, diff-check, 151 focused contracts, AST boundary tests, and rendered local browser component QA passed. Vercel Production deployment `dpl_3NbgJKi1ALV7iuTrCvTcd2knv1nm` for SHA `b0186d4` is Ready and production root returned 200; an authenticated Chrome session opened the existing Canvas and Storyboard Director without console warnings or errors. The current production project had no approved scene/beat/shot evidence, so the advisory controls could not truthfully be triggered; that exact interaction remains QA_HARNESS_LIMITATION, not a product failure. No crawler, external model, Provider/BYOK, generation, billing/payment/credit, schema, env, cn-executor, or Production DB behavior changed.)

Last updated: 2026-08-13 (P0-CREATOR-INTELLIGENCE-KNOWLEDGE-ENGINE-A VALIDATED / PRODUCTION_DEPLOYED -- commits `0eae2ac`, `b966cd6`, `0d1cccc`, `4f9377a`, `624e439`, and `df4e81a` establish the local, Creator City-owned knowledge foundation for future deterministic creative skills. It provides strict immutable knowledge-record validation, a six-rule approved cinematic pack, stable resolver receipts, and a capability map that strengthens existing skills in place. The static boundary guard now checks production TypeScript ASTs rather than test prose. Full type-check, lint (existing warnings only), build, 122 focused knowledge/skill contracts, 2 boundary tests, agent check, and diff-check passed. Production deployment `dpl_BJFyB6xN3zXcfZzgwo5Y5W6tsiBM` for implementation SHA `415e657` is Ready and aliased to `https://creator-city-vert.vercel.app`; read-only smoke returned 200 for `/` and the expected login redirect for `/create`. This foundation has no UI, network request, crawler, model, Provider/BYOK, generation, billing/payment/credit, schema, env, cn-executor, or Production DB behavior. Authenticated Production interaction is not applicable.)

Last updated: 2026-08-04 (CANVAS TOOL RELIABILITY QA VALIDATED / PREVIEW_DEPLOYED -- implementation commit `1cfa2ce` places the Shot Sequencer in the same fixed `z-[1199]` modal layer used by the other director panels, fixing a real Canvas-flow pointer interception that prevented its close action. The same change updates the stale NodeToolCenter assertion from the retired fixed-grid split to the live `分镜参考提取` entry and adds four isolated Preview browser contracts. A new dedicated Creator City Preview deployment `dpl_Bj9j2H8zeDkcJgCpCen1U79Rg1wi` is Ready at `https://creator-city-4un0m1z1y-aarons-projects-e26427fd.vercel.app`; all 9 Canvas safe-write paths passed. The suite proves Camera and Lighting source preview/settings/derived drafts/save/reload, selected-node Task/Tool/Asset navigation and menu boundaries, Shot Sequencer A -> cloud -> clean Browser B -> modify -> A reload, and 20/50/100 text-node creation. Browser measurements were 8.6s / 22.2s / 43.1s; autosave sent zero Canvas PUTs, each explicit save sent one Canvas PUT, and request capture recorded zero generate, Provider, payment, billing, credit, wallet, recharge, or checkout mutations and zero page errors. `pnpm type-check`, lint (existing warnings only), build, Canvas static boundary tests, `agent:check`, and diff-check passed. This is a disposable Preview write result; authenticated Production interaction remains a separate read-only QA limitation. No Provider/BYOK/generate/billing/payment/schema/env/cn-executor/Production DB change.)

Last updated: 2026-08-04 (P1-CANVAS-TOOL-PANEL-VIEWPORT-CONSTRAINT VALIDATED / CLOSED; P0-CANVAS-COMMERCIAL-MATURITY-GOLDEN-PATH-QA VALIDATED / CLOSED -- commits `7b1387c` and `25b9fd9` replace Keyframe Extractor's vertically centered floating panel with a 16px viewport-bounded shell, a scrollable body, fixed focus-node footer, dialog semantics, and a uniquely named close control. TDD first recorded the missing dialog/close/bounds behavior as RED; the final browser-backed component suite passed 40/40 across 1280x720 and 390x844 boundaries, keyframe provenance, cancellation, and fallback states. The safe Preview branch ran all five isolated persistence flows successfully: local image import, independent authenticated-context recovery, freeform reference extraction, local video image/video draft creation and refresh recovery, and local storyboard sketch-board recovery. Captured requests contained zero generate, Provider, payment, billing, credit, wallet, recharge, or checkout mutations, with zero page errors. Web type-check and build passed; lint passed with existing warnings only; Canvas render/layout contracts passed 9/9; `agent:check` and `git diff --check` passed. `main` is pushed at `25b9fd9`; Production serves the matching new CSS fingerprint, returns 200 for `/`, and redirects unauthenticated `/create` to login. Authenticated Production Canvas interaction and console capture remain QA_HARNESS_LIMITATION because the available automation context has no signed-in Chrome session; this is not a Production write claim. No Provider/BYOK/generate/billing/payment/schema/env/cn-executor/Production DB change.)

Last updated: 2026-08-04 (P0-CANVAS-COMMERCIAL-MATURITY-GOLDEN-PATH-QA VALIDATED / CLOSED_WITH_P1_TOOL_UI_FOLLOWUP -- commit pending; the new guarded Preview regression creates one disposable account and project, imports a local image, saves it, then opens the same project in a second independent authenticated Playwright context whose storage state intentionally has `origins: []`. The second context restored exactly one image asset node with zero page errors and zero generate, Provider, payment, billing, credit, wallet, recharge, or checkout mutations. Focused independent-context QA passed 1/1; image import, independent recovery, freeform reference extraction, and local storyboard sketch recovery passed 4/4; type-check, lint with existing warnings only, and build passed. The separate local-video keyframe regression timed out because `KeyframeExtractorPanel` can exceed a 1280x720 viewport, placing its generic close button above the visible viewport. This is a reproducible P1 PRODUCT_UI_ERROR, not a successful video-path claim; it is tracked as P1-CANVAS-TOOL-PANEL-VIEWPORT-CONSTRAINT. No product runtime, API, Provider, generation, billing/payment, schema, environment, cn-executor, or Production DB change.)

Last updated: 2026-08-04 (P1-CANVAS-LOCAL-VIDEO-IMPORT-AND-KEYFRAME-ENTRY VALIDATED / CLOSED -- commits `6d5d23e` and `5b45d7f` extend the existing local Canvas import path to accept MP4, WebM, and MOV files up to 20 MB, bind the uploaded asset identity to a recoverable Video node, and retain click-to-load video preview behavior. The image-only task-dialog reference validator remains image-only. A real Preview browser run created a disposable local WebM fixture, imported it into Canvas, opened the standard node `工具` menu, launched `关键帧提取`, created image and video continuation drafts, manually saved, and refreshed the project. The test found a missing NodeToolCenter entry; TDD captured the failure, then `5b45d7f` moved Keyframe Extractor from the old Asset menu into the standard Video node tools menu without a duplicate entry. Focused regression passed 43/43; type-check, lint with existing warnings only, build, and diff-check passed. The targeted Preview video flow passed after the deployed fix, and Vercel Preview plus Production for `5b45d7f` are Ready. The prior isolated image/reference/sketch flows remain independently validated at `4203078`; they were not re-claimed as a combined 4/4 run here. No Provider, auto-generation, billing, payment, credits, schema, environment, cn-executor, or Production DB change.)

Last updated: 2026-08-04 (P1-CANVAS-BROWSER-QA-HARNESS-RECOVERY VALIDATED / PREVIEW_MEDIA_PATH_PASS -- commit `4203078` fixes a real local-asset preview defect exposed by Preview QA: the media URL filter now permits only the owned `/api/assets/<assetId>/file` proxy and continues rejecting arbitrary `/api/*` URLs. The focused RED/GREEN regression passes, as do local-image contracts 24/24, web type-check, lint with existing warnings only, and web build. A current isolated Preview at the same commit passes 3/3 real browser flows: local image import and refresh recovery; arbitrary freeform reference extraction into a visible persisted node; and local storyboard sketch-board save/reload. Request capture asserts zero generation, Provider, payment, billing, credit, wallet, recharge, or checkout mutations and zero page errors. Keyframe Extractor's real Chromium panel interaction now passes 39/39 focused checks: frame preview, cancellation, unavailable-video/CORS timestamp-only fallback, image/video draft creation, stable provenance, and no frame bytes, source URLs, or false asset-generation claims. The full Canvas toolbar entry cannot be exercised under the Preview safe-write policy because the product accepts local images only and shows Keyframe Extractor only for an existing video result; adding local video import or a safe owned video fixture is a separate P1 product/QA task. This does not claim authenticated Production browser QA.)

Last updated: 2026-08-03 (CANVAS QA SPRINT 1-7 COMPLETE_WITH_LIMITATIONS -- the Founder-authorized local draft was restored, then synchronized before Production UI QA. `484bb83` is Production Ready and the left rail now passes Add -> Director -> Prompt -> close interaction: exactly one menu is visible at a time and console warnings/errors are zero. At 100% canvas zoom, compact nodes remain readable and the inspected task panel measures 480x420px within the 1514x810 viewport. Derived-tool entry points, source-immutability messaging, Camera/Lighting modal isolation, and disabled-no-account generation boundaries were verified without creating test nodes or invoking a Provider. `docs/CANVAS_TOOL_PANEL_PHASE4_AUDIT.md` records the non-duplicative upgrade sequence. 20/50/100 render/save scheduling contracts passed 18/18; video preview stays click-to-load with metadata preload. Reference extraction/local import contracts passed 33/33 plus 1/1 static boundary, but the full Preview crop/upload/node/save/reload remains QA_HARNESS_LIMITATION because it requires a disposable direct-drag image node with a stable assetId; no Production test asset was uploaded. No Provider, generation, payment, billing, schema, environment, cn-executor, or Production DB action occurred.)

Last updated: 2026-08-02 (P0-CANVAS-LOCAL-STORYBOARD-SKETCH-BOARD-V1 VALIDATED / CLOSED -- `f6818b5` lets an ordinary Text node with absent or null `metadataJson` start the existing Storyboard Director Recipe; malformed metadata remains a conflict. `048a5c0` adds the isolated Preview regression that creates the local black-and-white sketch board, approves the explicit review stages, adjusts then restores a frame, manually saves, refreshes, and reopens the same source node by stable canvas node ID. All three isolated Preview Canvas persistence checks pass: local image import, freeform reference extraction, and local storyboard sketch-board recovery. Request capture found no generation, Provider, payment, billing, credit, or wallet mutations, and no page errors. `pnpm type-check`, `pnpm lint` (existing warnings only), `pnpm build`, and `git diff --check` passed. Vercel Production deployment for `048a5c0` is Ready at `https://creator-city-vert.vercel.app`. This is a disposable Preview write QA result, not an authenticated Production interaction claim. No Provider/BYOK/generate/billing/payment/schema/env/cn-executor/Production DB change.)

Last updated: 2026-08-02 (P1-CANVAS-STORYBOARD-REFERENCE-EXTRACTOR-PREVIEW-WRITE-QA QA_HARNESS_LIMITATION -- the isolated Preview account, project, and a disposable uploaded image reference work; the reference persists in the task context without console errors or generation/payment traffic. The actual extractor deliberately requires an image node with both rendered media and a stable `assetId`. Dialog reference upload does not convert the current node into that source-node shape, and the available browser harness cannot perform the product's direct local-file drag-to-canvas gesture across desktop applications. Therefore crop/upload/derived-node/save/reload is not claimed as PASS and is not a Product Bug. The static local-only boundary test passes; the local component test runner cannot start because `tsx` is absent from the installed dependencies. No Production asset, database, environment, Provider, generation, billing, or payment operation occurred.)

Last updated: 2026-08-02 (P1-CANVAS-AUTO-GENERATION-REQUEST-AUDIT VALIDATED / NO_CODE_CHANGE -- static call-chain audit confirms Canvas mount sends one `GET /api/generate/image` and one `GET /api/generate/video` solely to read local Provider configuration and lightweight `ProviderAccount` status. Those GET handlers do not invoke a Provider, create a generation job, call billing, or mutate credits. `callGenerationApi` uses POST and is reachable only from explicit generate or regenerate handlers. Retain the status reads because they prevent the UI from offering unavailable configurations; browser request naming must not misclassify these local status checks as Provider generation.)

Last updated: 2026-08-02 (P0-CREDITS-WALLET-CREATION-RACE-RECOVERY PRODUCTION_DEPLOYED / PREVIEW_QA_PASS -- commit `8691fc8` makes the existing wallet bootstrap recover a concurrent unique-key collision by rereading the wallet created by the competing request. Balance, ledger, payment, Provider, generation, schema, environment, and Production DB semantics are unchanged. TDD recorded the missing collision-recovery helper as RED, then the focused regression passed 1/1; `pnpm --filter web type-check`, `pnpm lint` (existing warnings only), `pnpm type-check`, `pnpm build`, and `git diff --check` passed. Vercel Production deployment at `8691fc8` is Ready. The isolated Preview QA schema was restored from the existing Prisma schema using idempotent SQL only; a disposable account then registered, opened `/create`, created a text node, saved it, and recovered it after refresh with zero console errors. This verifies first-wallet initialization and basic Canvas persistence in Preview only; it does not claim Production browser QA or a forced concurrent collision reproduction. No Production database or environment was changed.)

Last updated: 2026-08-01 (P0-CANVAS-LOCAL-STORYBOARD-SKETCH-BOARD-V1 PRODUCTION_DEPLOYED / CLOSED_WITH_PREVIEW_WRITE_QA_PENDING -- commits `193c6ea`, `6cd123c`, `fb46233`, and `d08dd1a` add a local, deterministic black-and-white storyboard sketch board to the existing Storyboard Director Recipe, rather than a parallel tool. After approved shot review, the user can create one bounded board, adjust composition, camera angle, subject anchors, action line, and movement for a single frame, then explicitly restore that frame to the approved local derivation. Every mutation uses the existing identity-guarded Recipe commit path and local/canvas persistence scheduling; stale source or shot review keeps frames visible but disables editing. Recipe V1 data upgrades safely without a board; V2 board data validates exact approved-shot order, immutable derivation, render keys, revision identity, bounded fields, and stale state. The focused regression suite passed 127/127, static local-only boundary checks 2/2, `pnpm type-check`, `pnpm lint` (existing warnings only), `pnpm build`, and `git diff --check` passed. Vercel Production deployment `dpl_AdKDiASmv1vXFbQ3LG4BzjGPYAHD` is Ready at SHA `d08dd1a8c319158c26231130b26e62d1ccd44e23` and aliases `https://creator-city-vert.vercel.app`. Component browser regression validates create, targeted adjustment, stale labeling, restore, disabled controls, and zero network calls; full authenticated create/save/refresh on an isolated Preview fixture remains intentionally pending. No Provider/BYOK/generate/billing/payment/schema/env/cn-executor/Production DB change.)

Last updated: 2026-08-01 (P0-CANVAS-STORYBOARD-REFERENCE-EXTRACTOR-V1 PRODUCTION_DEPLOYED / CLOSED_WITH_PREVIEW_WRITE_QA_HARNESS_LIMITATION -- commits `324459c` through `da5e868` replace the live fixed-grid storyboard split entry with one freeform, source-linked reference extractor; legacy persisted grid-split metadata remains readable and unchanged. The extractor starts with zero selections, accepts arbitrary image regions, creates V2 crop provenance with stable source asset/node/session identity, rejects foreign, missing, and cross-project source assets before storage upload, and retries a pending node creation without recropping or re-uploading. Source assets remain immutable; confirm, close, and backdrop behavior are safe during processing. Focused regression passed 88/88; the static boundary test, `pnpm type-check`, `pnpm lint` (existing warnings only), `pnpm build`, and `git diff --check` passed. Vercel Production deployment `dpl_68e9mecVweVLVxKYD5tt4LpnYeZs` is Ready at SHA `da5e868222ecd73cea96a7191b3ba5facf47161e` and aliases `https://creator-city-vert.vercel.app`. Authenticated Production Chrome restored the Founder-authorized local draft, confirmed the protected no-asset image node does not surface the asset-required extractor, and recorded zero console errors. Isolated Preview QA proved reference upload but could not create the required source-asset node because the available harness cannot perform the product's direct local-file drag-to-canvas gesture; crop/upload/save/reload remains unclaimed rather than failed. No Production asset upload, Provider call, generation, payment, billing, schema, environment, or Production DB mutation was performed.)

Last updated: 2026-08-01 (P1-CANVAS-COMPACT-NATIVE-SCALE PRODUCTION_DEPLOYED / WAITING_VISUAL_QA -- commit `8829f88` establishes one shared compact layout contract: new nodes use readable 236–248px dimensions at 100% canvas zoom, and legacy nodes with only the previous default dimensions are normalized on load without changing non-default custom sizes. The task dialog is 480×420px on desktop, contracts to 342×320px at 390px width, retains an internal scroll boundary, and recomputes its placement on viewport resize. Focused layout/result-quality regression passed 35/35; `pnpm type-check`, `pnpm lint` (existing warnings only), `pnpm build`, and `git diff --check` passed. Vercel deployment `dpl_FH874oaWNye19tdnvA6sWJPU9htG` is `target=production / Ready` and aliases `https://creator-city-vert.vercel.app`; unauthenticated `/create` correctly redirects to login. Local Python Playwright is unavailable and the authenticated production canvas held a user-local draft recovery choice, so visual interaction QA remains `QA_HARNESS_LIMITATION`, not PASS. No Generate/Provider/BYOK/Billing/Schema/cn-executor/package/env/Production DB change.)

Last updated: 2026-08-01 (P1-CANVAS-TOOL-RESULT-QUALITY-LAYER VALIDATED / CLOSED_WITH_PRODUCTION_QA_LIMITATION -- commits `e943cf3`, `c09a519`, `909c6e8`, `475d022`, `b50286a`, `5f209ef`, `c69f07a`, `4490230`, and `816a000` add one compact, read-only result-quality strip to Grid Split, Annotation, Color Grade, Continuity, Variant Planner, A/B Compare, and Keyframe Extractor. Statuses are backed by existing evidence: previews, requests, saved annotations, actual split children, categorical continuity findings, chosen A/B winner, and cancellation-safe local keyframe extraction. No fake quality scores remain in Continuity UI or copied reports. Focused browser-backed regression passed 30/30; `pnpm type-check`, `pnpm lint` (existing warnings only), `pnpm build`, and `git diff --check` passed. Production Chrome opened the existing authenticated Canvas but stopped at its local-draft recovery confirmation because either option changes user state; that is recorded as QA_HARNESS_LIMITATION, not Production PASS. No Generate/Provider/BYOK/Billing/Schema/cn-executor/package/env/Production DB change.)

Last updated: 2026-08-01 (P1-CANVAS-CREATIVE-WORKFLOW-POLISH VALIDATED / CLOSED -- implementation `0150cb9` strengthens the existing Storyboard Director and NodeToolCenter without adding a parallel UI: the Director renders one compact, deterministic next-action row for stale-source, partial-batch, review, blocker, and ready-to-materialize states; NodeToolCenter renders one capability-aware recommended next action above its unchanged categorized tools. TDD recorded missing-helper and missing-rendered-recommendation RED states; final focused regression passed 84/84. `pnpm type-check`, `pnpm lint` (existing warnings only), `pnpm build`, and `git diff --check` passed. Authenticated Production Chrome read-only Canvas QA opened an existing persisted split-image node and its Tools menu: exactly one `推荐下一步` section and one `摄影机控制 建议` button rendered while the ordinary 摄影机控制, 分镜拆格, 画面标注, and 调色盘 entries remained available; console warning/error count was zero. No save, upload, generate, Provider, payment, or tool action was run. No Generate/Provider/BYOK/Billing/Schema/cn-executor/package/env/Production DB change.)

Last updated: 2026-07-31 (P0-CANVAS-PENDING-BROWSER-QA-CONSOLIDATION PARTIAL / READ_ONLY_SMOKE_PASS -- authenticated Production Chrome QA reopened the existing Golden Path projects without creating, saving, uploading, generating, or invoking Provider/payment actions. The main project rendered its persisted node graph, four storyboard split children, annotation badge, image/video nodes, and explicit local-draft recovery prompt; the prompt was deliberately not answered because either choice can alter local recovery state. A second persisted asset project loaded normally with its Image node and cloud-synced state. Its Director and Prompt tool menus opened, node `任务 / 工具 / 资产` entry opened, local-reference context rendered, the absent Provider account left Generate disabled, and Camera Control opened and closed cleanly without creating a derived version. Dashboard reported two projects and six assets; Asset Library rendered six READY images with the expected project grouping. Console warn/error count was zero on Canvas, Asset Library, Dashboard, and Camera Control checks. Remaining QA requires an isolated disposable project for save/recovery/derived-node writes, plus independent-browser recovery; it is not claimed by this read-only run. No product code, API, schema, Provider/BYOK, generation, billing/payment/credits/wallet, cn-executor, package/lockfile, env, or Production DB change.)

Last updated: 2026-07-31 (P0-CANVAS-STORYBOARD-SCRIPT-V1 COVERED_BY_STAGE_C / CLOSED -- a new design audit confirmed that the requested Text-node script-to-storyboard workflow is already implemented by Creator Skill Engine Stage C's Storyboard Director Recipe. The existing Recipe runs `script-segmentation`, then `narrative-beat-analysis`, then `shot-planning`; it gates every transition on explicit approval, preserves source immutability and evidence, marks downstream work stale with exact impact after upstream edits, and separately exposes grouped materialization, storyboard-board synchronization, and compatible image/video draft creation. Stable Recipe/materialization identities, persisted receipts, explicit partial-batch recovery, and Canvas save/reload already cover repeat and recovery behavior. Fresh focused regression passed 119/119 across Recipe progression, identity, persistence, intelligence, materialization, synchronization, draft dedupe, and partial failures. No product code, API, schema, Provider/BYOK, generation, billing/payment/credits/wallet, cn-executor, package/lockfile, env, or Production DB change.)

Last updated: 2026-07-31 (P1-CANVAS-E2E-REGRESSION-HARNESS VALIDATED / CLOSED_WITH_PREVIEW_FIXTURE_PENDING -- implementation commits `b7ac05e`, `c52ff50`, and `7d973ae`, following the approved design and plan in `docs/superpowers/specs/2026-07-31-canvas-e2e-regression-harness-design.md` and `docs/superpowers/plans/2026-07-31-canvas-e2e-regression-harness.md`. The dedicated `apps/web/playwright.canvas.config.ts` discovers only `apps/web/tests/e2e` and writes artifacts outside the worktree at `/tmp/creator-city-canvas-e2e`, so Node unit tests and QA artifacts cannot contaminate the browser release gate or Git status. The credential-free `/create` preflight records method and pathname evidence only, verifies login redirect, and fails on generation, payment, billing, credit, wallet, recharge, or checkout mutations. The authenticated save/select/reload path is fail-closed: it cannot navigate or write unless an explicit non-production Preview base URL, isolated storage state, safe project id, `PLAYWRIGHT_SAFE_ENV=preview`, and `PLAYWRIGHT_ALLOW_SAFE_WRITES=1` are all supplied; the Production Canvas domain is rejected. TDD recorded missing-helper and missing-suite RED states. Final safety tests passed 6/6, static boundary tests 3/3, isolated Playwright discovery listed exactly two specs, and the local unauthenticated browser preflight passed 1/1. `pnpm type-check`, `pnpm lint` (existing warnings only), `pnpm build` (84 pages), and `git diff --check` passed. The authenticated Preview save/reload fixture is intentionally pending and is not claimed as executed. No product runtime, Generate, Provider/BYOK, billing/payment/credits/wallet, schema/migration, cn-executor, package/lockfile, env, or Production DB change.)

Last updated: 2026-07-31 (P1-PROJECT-SUMMARY-ASSET-COUNT-CONSISTENCY PRODUCTION_DEPLOYED / CLOSED_WITH_QA_LIMITATIONS -- approved design and implementation plans are recorded in `docs/superpowers/specs/2026-07-31-project-summary-asset-count-consistency-design.md` and `docs/superpowers/plans/2026-07-31-project-summary-asset-count-consistency.md`. The project list and Dashboard now count only direct `Asset` rows owned by the current user and bound to the displayed `projectId`, matching `/api/assets`; `ProjectAsset` associations no longer create duplicate card counts. The recent-project fast path, owned-project list, and mixed project list use one batched `db.asset.groupBy` lookup per response, and isolated count-query failures degrade to zero with an `asset_count_query` warning while preserving the list response. TDD recorded four RED helper tests; final focused helper tests passed 9/9 and static route checks 2/2. `pnpm type-check`, `pnpm lint` (existing warnings only), `pnpm build` (84 pages), and `git diff --check` passed. Vercel Production is Ready at SHA `78812d8`. Authenticated production Chrome UI comparison of `/projects`, `/dashboard`, and filtered `/assets` could not complete because two read-only page-snapshot attempts timed out before page content was available; this is recorded as `QA_HARNESS_LIMITATION`, not a product PASS or product error. No asset write, schema/migration, Canvas save, generation, Provider/BYOK, billing/payment/credits/wallet, cn-executor, package/lockfile, env, or Production DB change.)

Last updated: 2026-07-31 (P1-CANVAS-PERFORMANCE-AND-REQUEST-STORM-CLEANUP PRODUCTION_DEPLOYED / CLOSED_WITH_QA_LIMITATIONS -- implementation commits `1a8d2d8` and `8f6e36e` isolate the canvas node layer and replace per-edge node searches with a memoized node index, while preserving the existing explicit-save and click-to-load-video behavior. Pure 20/50/100-node render-planning, canvas-save scheduling, and canvas-save-integrity tests passed 25/25; performance/request-storm static tests plus save-integrity static tests passed 10/10; type-check and build passed; lint exited 0 with existing warnings only; diff-check and forbidden-zone audit passed. Vercel Production deployment `CikrM7JYVLQAqqLrbfqE5vwD6jFh` is Ready at `fb16b44`, whose tree includes the implementation. Authenticated Production Chrome read-only smoke loaded the existing Golden Path project with 14 nodes, 7 image nodes, 1 video node, 5 mounted images, 0 mounted video elements, and zero console warnings/errors; it did not create, save, upload, generate, or mutate data. Local `/create` interactive QA remains blocked by missing local auth/database context. Exact browser timing/request counts remain a QA harness limitation. No generate route, Provider/BYOK behavior, billing/payment/credits/wallet, schema/migration, cn-executor, package/lockfile, env, or Production DB change.)

Last updated: 2026-07-31 (P1-CANVAS-STORYBOARD-GRID-DETECTION-CONFIDENCE-GUARD VALIDATED / CLOSED -- commits `97ee8cf` and `8e2e0dc`, with approved design/plan commits `fb9dbbb`, `e1e54c1`, and `bf432b6`. The existing Grid Split tool is strengthened in place: detector evidence now requires local boundary contrast and 90% line continuity, and automatic selection additionally requires reliable horizontal and vertical evidence; a single-axis or ambiguous candidate is shown only as a suggested layout and requires an explicit user click before crop/upload. The panel resets selection on source change, renders no crop overlay/items for an unconfirmed layout, and disables `裁切并入库` until a layout is confirmed or manually selected. TDD recorded 4 RED detector assertions plus one RED static panel boundary; focused regression passed 21/21 and static checks 2/2; type-check, lint (existing warnings only), build, and diff-check passed. Authenticated Production Chrome QA PASS: the existing source opened the Grid Split panel, unconfirmed state showed the helper and disabled crop, manual `2x2` rendered exactly four preview labels and enabled crop, closing the panel left no upload or generation state. No crop, upload, Provider, generation, billing/payment, schema, env, package, cn-executor, or Production DB mutation was performed.)

Last updated: 2026-07-31 (P0-CANVAS-TOOL-SEMANTIC-INTEGRITY-A PRODUCTION_DEPLOYED / CLOSED_WITH_QA_LIMITATIONS -- implementation commits `1c650cd`, `5a8e6e6`, and `da87e48`; documentation/spec commits `62de9ef`, `b79dd86`, and `72a5fcb`. Storyboard split children now derive a deterministic canvas position from their persisted row/column identity and use collision-safe placement; the vertical row step is 345px, which exceeds the workspace's strict 24px collision gap. TDD recorded the missing-helper RED, the legacy fake-editor static RED, and the 3x2 overlap RED before each corresponding fix. The focused suite passed 28/28, semantic static checks 2/2, type-check, lint (existing warnings only), build (84 pages), and diff-check. Authenticated Production Chrome loaded the existing Golden Path QA project, chose the server version on the recovery prompt, and rendered the persisted source result plus four saved storyboard assets; the deployed bundle contains none of the removed legacy editor labels. This closeout did not create a new Production asset or node, invoke Provider/generation/payment, or alter project data. Local `/create` browser access remains an AUTH_BLOCKER; exact browser request counts/resource timing are QA_HARNESS_LIMITATION; Vercel CLI inspection is unavailable because of local npm-cache permissions and no cache change was made. No generate route, Provider adapter, BYOK semantics, billing/payment/credits/wallet, schema/migration, cn-executor, package/lockfile, env, or Production DB change.)

Last updated: 2026-07-29 (P0-ASSET-REMIX-PHASE-1A-INTERNAL-INGEST-SECURITY-QA VALIDATED / CLOSED -- production-hidden internal ingest endpoint remains fail-closed. The focused security contract suite passed 31/31, covering the dual feature gate, valid/missing/wrong/stale/tampered HMAC input, malformed transform identity, traversal and canonical-key violations, artifact/hash/confidence/ownership/MIME/PNG/alpha/dimension failures, idempotency, source immutability, and the absence of generate/provider/credits/payment calls. Three non-secret Production POST canaries (missing headers, fake HMAC, and tampered body) each returned 503 while the gate was disabled; the route returns before request-body read, HMAC, database, or object-storage access. No environment variable was enabled, no source Asset or Canvas node was changed, and no Provider, generation, payment, credit, or database write occurred. Railway/RunPod staging may proceed only under its separate authorized task; public release remains blocked by staging, alpha, and Founder approval.)

Last updated: 2026-07-29 (P0-PROD-AUTH-DATABASE-RECOVERY VALIDATED / CLOSED -- Production `auth_me_db_error` root cause confirmed as the WRXZ Supabase Free Plan project being paused/restoring, not a malformed Vercel connection string or a product-code regression. Supabase project `wrxzkibitagojunfphag` recovered to Healthy; its official Tokyo Transaction Pooler template matches the existing Vercel `DATABASE_URL` scope, so no environment variable, schema, code, migration, Production DB write, deploy, or secret rotation was needed. Production canaries returned `/api/auth/me` 200, `/api/skills/health` 200, and unauthenticated `/api/projects` 401 as expected; Vercel startup logs now report the complete migration state and no longer report `tenant/user not found`. Authenticated Chrome QA loaded the existing canvas project and protected Dashboard with project, asset, task, and Provider data; final console warn/error count was 0. The earlier recoverable warning is resolved; retain a future P1 review of runtime migration work in `instrumentation.ts` as a performance concern only.)

Last updated: 2026-07-27 (P0-CANVAS-CREATOR-SKILL-ENGINE-STAGE-C STAGE_C_VALIDATED / CLOSED_WITH_QA_LIMITATIONS — implementation chain through `c225bbc38d56bcdf4a5f87dc23372922c62caee4`; exact Vercel Production deployment `creator-city-i7e0byd8i-aarons-projects-e26427fd.vercel.app` is READY at SHA `c225bbc`; Stage C strengthens the existing Storyboard Director with a persistent Recipe workflow instead of adding a duplicate tool: source Text -> scene review -> narrative-beat review -> shot review -> deterministic Intelligence -> grouped materialization, shot-board synchronization, and compatibility draft creation; independently callable Script Segmentation, Narrative Beat Analysis, and Shot List Builder remain available; authenticated Production Google Chrome QA found and fixed one release blocker where the public tool created a control node while a legacy `STORYBOARD_TOOLS_ENABLED=false` gate hid the workspace; after the fix, repeat invocation reused the same control node, all three review stages completed, Intelligence reached `可落地`, grouped materialization added exactly 3 nodes and repeat apply added 0, compatibility creation added exactly 2 image/video drafts and repeat added 0, shot-board sync produced 2 Recipe shots, a manual third shot survived repeat sync, approved-scene editing showed the exact `1 个节拍和 2 个镜头失效` impact preview and cancel restored the persisted value, manual cloud save plus refresh restored 20 nodes, the approved Recipe, and all 3 board shots; video submission remained intentionally disabled and no real Provider/generation/payment action was performed; focused executable/rendered tests 512/512 plus static boundaries 72/72 PASS, including 20/50/100-node persistence, 40-scene bounds, responsive 390x844/768x1024/1280x720/1440x900 layout, focus containment, recovery, partial-batch, dedupe, and source-immutability coverage; type-check, lint (36 existing warnings only), build, agent-check, and diff-check PASS; exact Chrome request capture, screenshots, temporary viewport override, and independent authenticated browser recovery remain `QA_HARNESS_LIMITATION` / `AUTH_BLOCKER` and are not claimed as PASS; Production console logged repeated recoverable `auth_me_db_error` warnings while retaining the active session, recorded as a P1 Auth/DB stability follow-up; no generate route, Provider adapter, BYOK semantics, billing/payment/credits/wallet, schema/migration, cn-executor, package/lockfile, env, or Production DB change)

Last updated: 2026-07-18 (P0-CANVAS-CREATOR-SKILL-ENGINE-STAGE-B STAGE_B_VALIDATED / CLOSED — implementation chain `28f750e` through `5601e1d5be1ebe35dd46005b772951187356f5db`; Vercel Production deployment `creator-city-1ui2k71kb-aarons-projects-e26427fd.vercel.app` is READY at exact SHA `5601e1d`; Stage B adds independently callable `narrative-beat-analysis` and upgrades the existing Shot List Builder to public `shot-planning` without adding a duplicate tool; approved Artifact handoff, reviewed edit/order/remove/rerun, grouped materialization, compatibility image/video drafts, stable deduplication, async generation abort guards, and viewport-aware NodeToolCenter placement are implemented; authenticated Production Google Chrome QA PASS: Narrative Beats carrier opened Shot List without `SHOT_SOURCE_CONFLICT`, 2/2 shots were approved, grouped apply created `Scene 1 · Shot Plan` exactly once (`11 -> 12`) and repeat apply stayed at 12 with `已存在并跳过镜头规划：scene-001`; compatibility creation added one image and one video draft (`12 -> 14`) and repeat stayed at 14 with both shot IDs skipped; generation displayed the required second confirmation and was cancelled before any Provider call; direct plain Text -> Shot List remained available; manual save reached `已同步到云端`, server-version reload restored 14 nodes and all three created titles, and Chrome console warn/error count was 0; exact network request capture and screenshots are unavailable in the Chrome extension and remain `QA_HARNESS_LIMITATION`; Preview authenticated QA remains `AUTH_BLOCKER` because Preview uses an independent cookie domain, while both Preview and Production deployments were READY; final focused tests 73/73 and static Stage B boundaries 44/44 PASS, with the broader Stage A/B suite 310/310 PASS before the final source-snapshot follow-up; type-check, lint (existing warnings only), build, and diff-check PASS; no generate route, Provider adapter, BYOK, billing/payment, schema/migration, cn-executor, package/lockfile, env, or Production DB change; Stage C was not started)

Last updated: 2026-07-17 (P0-CANVAS-CREATOR-SKILL-ENGINE-V1 STAGE_A_VALIDATED / CLOSED — implementation commit `2936cc15690c7239d7df11c87dac6118d3beed50`; Vercel deployment `creator-city-6etqruz9c-aarons-projects-e26427fd.vercel.app` is READY / Production at exact SHA `2936cc1`; Stage A adds a deterministic, independently callable Creator Skill runtime, immutable analysis/apply identity contracts, the `script-segmentation` Skill v1.0.0, approval-bound apply planning, generic review UI, and Text-node canvas integration; authenticated Production Google Chrome QA PASS: a two-scene script produced 2 review drafts with evidence and 0 preselected approvals, one edited/approved scene created exactly one derived Text node plus `§ 剧本分场` edge, source text remained unchanged, a repeated apply created no node and reported `已存在并跳过：scene-001`, manual cloud save reached `已同步到云端`, and refresh restored 10 nodes, source/derived text, and the Skill edge label; unheaded prose returned `需要审核` with `FALLBACK_SCENE_BOUNDARIES`, closing discarded an unsaved title edit, and Chrome console warn/error count was 0; browser network capture is unavailable in the Chrome harness, so exact request counts remain `QA_HARNESS_LIMITATION`, while static boundaries and the exercised flow made no Generate, Provider, billing, credits, wallet, payment, recharge, checkout, or upload call; local authenticated browser QA is `ENV_BLOCKER` because local `DATABASE_URL` is absent and Production DB credentials were not borrowed; targeted tests 167/167 and static UI/integration tests 20/20 PASS; type-check, lint (existing warnings only), build, diff-check, and forbidden-zone audit PASS; no schema/migration/env/package/lockfile/next.config/generate route/Provider/BYOK/billing/payment/cn-executor/Production DB change; later Skills, Stage B, and Storyboard Director Recipe were not started)

Last updated: 2026-07-15 (P0-CANVAS-COMMERCIAL-MATURITY-GOLDEN-PATH-QA PARTIAL / SECOND_BROWSER_PENDING — authenticated Production Google Chrome QA now covers the result-asset path after Founder manually dragged `creator-city-logo.png` into the dedicated `Golden Path QA 2026-07-15` project; the source image became a real result-bearing Image node, cloud save reached `已同步到云端`, and refresh restored it; AnnotationPanel PASS with pen, arrow, rectangle, ellipse, text `QA`, movement line, color change, width change, undo, metadata save, `已标注 6` badge, cloud sync, refresh recovery, and clear-without-save/cancel recovery; Storyboard Grid Split PASS after a manual 2x2 override: four crops uploaded as READY Assets, four lineage-bearing child Image nodes were placed on canvas, one unconfirmed save was retried explicitly, and refresh restored all four crop nodes, source annotations, and `已同步到云端`; the main project now restores 8 nodes and 5 real image results, `/assets` reports 6 READY image assets across the two QA projects, and `/tasks` remains 0; no Generate, Provider, billing, credits, wallet, payment, recharge, or checkout action was initiated; Chrome console warn/error count remains 0; exact request methods/status counts remain QA_HARNESS_LIMITATION; toolbar activation required keyboard Enter in the automation harness after locator click only closed the menu and is classified QA_HARNESS_LIMITATION because the panel and full workflow worked once activated; independent authenticated Chrome context is still unavailable, so cross-browser recovery is not claimed; P1 findings: project/dashboard asset-count consistency remains pending, and automatic grid detection reported 3x3 at 93% confidence for a non-grid logo although manual layout override worked; no product code, API, schema, migration, env, package, Provider, billing/payment, cn-executor, or Production DB direct change)

Last updated: 2026-07-15 (P0-CANVAS-LOCAL-DRAFT-RECOVERY-VERSION-GUARD VALIDATED / CLOSED — implementation `0369fb6`, Production-QA follow-up `d8b5441`, Vercel Production deployment `dpl_8oB11N4Zni9k9REoAuHAUeLSBhzJ` READY at exact SHA `d8b5441416720c845ba16a82d3ba1cbe37092b78`; root cause: authorized load automatically rendered a timestamp-newer local candidate, cross-project records were not rejected during normalization, and unconditional pagehide/beforeunload flushes rewrote clean server state as a newer unsynced draft; fix: matching project/workflow identity guard, server-first authorized load including empty canvases, explicit local recovery choice, synchronized server-version dismissal, and unsynced-change tracking so clean page leave does not manufacture drafts; TDD evidence: pure recovery/save tests 14/14 PASS and static recovery/save integration tests 12/12 PASS, with recorded RED failures before both implementation phases; `pnpm type-check`, `pnpm lint`, `pnpm build`, and `git diff --check` PASS, lint contains only existing warnings; authenticated Production Google Chrome QA: `Golden Path QA 2026-07-15` always rendered its 2 server nodes while stale 1-node/2-node local candidates appeared only as an explicit prompt, choosing `使用服务器版本` then hard-refreshing preserved 2 nodes, `已同步到云端`, and no prompt; `Golden Path Asset QA 2026-07-15` independently preserved its 1 image node and also reloaded cleanly after choosing the server; switching back restored the 2-node project with no prompt, proving project isolation; Chrome console warn/error count 0; one transient `ERR_TIMED_OUT` during QA recovered on retry and is QA_HARNESS/NETWORK_LIMITATION, not a product 5xx; exact browser request counts remain QA_HARNESS_LIMITATION, while no Generate, Provider, billing, credits, wallet, payment, recharge, or checkout action was initiated; screenshot `/tmp/creator-city-canvas-draft-recovery-final.png`; no API, schema, migration, env, package, lockfile, next.config, cn-executor, Provider, billing/payment, or Production DB change; P0-CANVAS-COMMERCIAL-MATURITY-GOLDEN-PATH-QA is unblocked and ready to resume, but was not automatically started)

Last updated: 2026-07-15 (P0-CANVAS-SAVE-INTEGRITY-HARDENING VALIDATED / CLOSED — implementation commit 4334162; canvas cloud saves now fail closed on partial node, edge, or deletion failures; the PUT route reserves the workflow version and returns `409 CANVAS_SAVE_CONFLICT` for stale writes, uses a 45s server deadline, and no longer returns while its own save promise can continue writing; the client uses a 55s timeout, sends the last acknowledged `baseUpdatedAt`, treats non-OK/partial responses as failures, clears deletion queues only after acknowledged success, and drains one pending save after the active save finishes; TDD evidence: helper test first failed with MODULE_NOT_FOUND and static contract first failed 5 assertions, then 7/7 helper tests and 5/5 static tests passed; type-check PASS; lint PASS with pre-existing warnings only; build PASS for 84 pages; diff-check and forbidden-zone checks PASS; authenticated Production Chrome QA PASS on creator-city-vert.vercel.app: prompt save/reload restored exact content, add/save/delete/save/reload preserved one expected Text node, duplicate manual click was blocked while save was in flight, and product console errors/warnings/409/500/503 were zero; exact request counts were unavailable from the Chrome harness and are classified QA_HARNESS_LIMITATION, while no generation, Provider, billing, credits, wallet, payment, or recharge action was initiated; Vercel production deployment dpl_3XQbHK9HTCSzGdtmkpwuYjRWztdb reached READY at implementation SHA 4334162710d5f20453a9d617890dc7b6ace13aca; no schema, migration, generate route, Provider adapter, BYOK, billing/payment, cn-executor, package, lockfile, next.config, env, or Production DB change)

Last updated: 2026-07-15 (P0-CANVAS-COMMERCIAL-MATURITY-NEXT-AUDIT AUDIT_COMPLETE / FOUNDER_DECISION_REQUIRED — source-level commercial maturity audit completed with no product code or runtime configuration changes; recent authenticated Production Chrome evidence confirms login, `/create`, safe Text-node creation, manual save/reload, project pages, assets/tasks, BYOK boundaries, storyboard split asset recovery, and consistent project node counts; release verdict: controlled development/demo may continue, unrestricted public beta is blocked by canvas save integrity; P0 findings: partial node/edge writes return HTTP 200 and client reports `已同步到云端`, failed deletions are ignored, queued in-flight save is cleared without being drained, and 15s client/20s route/52s write deadlines allow late ambiguous writes; P1 findings: camera/lighting context is localStorage-only and not cross-browser, reload recovery guard is not project-scoped, 20/50/100-node performance baseline is missing, no web E2E harness exists, and legacy browser-QA rows are fragmented; normalized next task is P0-CANVAS-SAVE-INTEGRITY-HARDENING pending Founder approval; fresh Chrome navigation attempt was QA_HARNESS_LIMITATION at about:blank and is not classified as a product failure; audit report: docs/CANVAS_COMMERCIAL_MATURITY_NEXT_AUDIT.md; no generate/provider/BYOK/billing/payment/schema/env/package/cn-executor/Production DB change)

Last updated: 2026-07-15 (P1-PROJECT-SUMMARY-NODE-COUNT-CONSISTENCY VALIDATED / CLOSED — implementation commit 395db72; retained the resilient owned-project list query and added one isolated batched CanvasWorkflow node-count query for all returned workflow IDs; `/projects` and `/dashboard` now report persisted node totals instead of hardcoded zero, while a count-query failure still returns the project list with a `node_count_query` warning and zero fallback; TDD evidence: helper test first failed with MODULE_NOT_FOUND, static route contract first failed on the missing import, then 5/5 helper tests and 1/1 static route test passed; type-check PASS; lint PASS with pre-existing warnings only; build PASS for 84 pages; diff-check PASS; forbidden-zone diff PASS; authenticated Google Chrome Production QA PASS on creator-city-vert.vercel.app: QA project `1da1de94...` reports 1 node in project center, Dashboard recent projects, and project overview; empty QA project remains 0; reload preserves all three counts; product Console errors=0 and warnings=0; direct JSON API navigation was blocked by the Chrome extension and classified QA_HARNESS_LIMITATION, not a product failure; no canvas write, generation, Provider, billing, credits, wallet, payment, schema, migration, env, package, lockfile, next.config, cn-executor, or Production DB change)

Last updated: 2026-07-14 (P0-PROD-CONTINUITY-CHROME-BROWSER-QA-CLOSEOUT VALIDATED / CLOSED_WITH_P1_FOLLOWUPS — authenticated Google Chrome QA PASS on creator-city-vert.vercel.app after the WRXZ Production cutover; dedicated QA registration/login PASS; `/create` and fresh project open PASS; one Text node created without generation, manual cloud save PASS, refresh/reopen restored the node and exact prompt; project overview reports 1 node; `/assets` and `/tasks` load successfully with expected empty states; `/dashboard`, `/projects`, and `/account/providers` load authenticated; BYOK center exposes no saved key and no Provider request was made; payment remains server-side fail-closed and no recharge request was submitted; Chrome product console errors=0 and warnings=0; Provider requests=0, generation POSTs=0, billing/credits/wallet/payment mutations=0; visual screenshot captured in Codex; sampled authenticated page transitions completed without hangs; P1 follow-ups recorded: dashboard/project-center summary cards show `0 nodes` while the same project overview correctly shows `1 node`, and the disabled-payment billing page still visually offers recharge submission buttons even though the server rejects writes; Codex Chrome plugin cache required a local runtime compatibility repair because its bundled bootstrap tried to overwrite the read-only `process` global; one dedicated Production QA user, two QA projects, and one Text node now exist in fresh WRXZ; no Creator City product code, API, schema, migration, generate route, Provider adapter, billing/payment code, cn-executor, package, lockfile, env file, or Production DB migration changed during browser closeout)

Last updated: 2026-06-29 (P0-CANVAS-STORYBOARD-GRID-SPLIT-V1 VALIDATED / CLOSED — commits 1a78671 + e81c1ca + a4c9506 + 2a460df + 7a91703; image-node storyboard grid split available from node asset/tool center; V1 layouts: 1x2, 2x1, 2x2, 3x2, 2x3, 3x3, 4x3; production blocker fixed: /api/assets/upload project validation now preserves ownership checks, retries transient Prisma P2024 pool timeouts, and returns explicit retryable errors instead of generic project-check failure; crop upload format changed from PNG to JPEG with matching .jpg filename to avoid 60s production upload fetch failures; metadata remains allowlisted only for toolId=storyboard-grid-split, with no arbitrary metadataJson passthrough; production QA PASS on creator-city-vert.vercel.app: panel opens, source image readable through media proxy, four crops upload with /api/assets/upload 201, four child image nodes created with 分镜拆格 lineage, cloud save completed, reload restored all 4 crop nodes, /assets found latest 图片·1-4 assets; video upstream did not auto-mount <video>; tests PASS: 16/16 targeted storyboard/upload tests, type-check PASS, lint PASS (pre-existing warnings), build PASS, diff-check PASS; boundary CLEAN: no generate/provider/billing/payment/schema/cn-executor/package/lockfile/next.config/env/Production DB direct changes; residual non-blocking finding: production canvas save/load still emitted transient Prisma P2024/503 during QA and should be handled by P1-CANVAS-PERFORMANCE-AND-REQUEST-STORM-CLEANUP)

Last updated: 2026-06-29 (P1-PUBLIC-FACING-POLISH-PACK-A VALIDATED / CLOSED — implementation commit ce8f399; homepage SoftAurora now preflights WebGL and falls back to silent CSS aurora when WebGL is unavailable, with production browser QA PASS and screenshot `/tmp/homepage-fallback.png`; marketplace public read fixed: unauthenticated GET /api/marketplace/listings returns public ACTIVE listings only, seller mine lookup remains auth guarded, and /marketplace no longer calls /api/me/marketplace-orders before auth; production QA PASS: marketplace page unauth 0 personal-order requests, 0 page 401s, listings 200, protected buyer orders 401, seller mine lookup 401, returned listings ACTIVE-only with no sensitive seller keys; community public copy cleaned: "前端预览阶段 / 动态数据 / mock" removed, production screenshot `/tmp/community-production-copy.png`; tests PASS: public-facing static 4/4, type-check PASS, lint PASS (warnings pre-existing), build PASS, diff-check PASS; boundary CLEAN: no canvas/create/generate/provider/billing transaction/payment create/schema/cn-executor/package/lockfile/next.config/Production DB changes; NEXT remains P0-CANVAS-REFERENCE-IMAGE-TO-PROVIDER-CONTEXT IMPLEMENTED_ON_PREVIEW / REAL_PROVIDER_QA_REQUIRED and P0-PAYMENT-CREDIT-INTEGRITY-HARDENING-B2 PUSHED_FOR_REVIEW_ONLY / BLOCKED_BY_NO_READONLY_DATABASE)

Last updated: 2026-06-28 (P0-PUBLIC-LAUNCH-PAYMENT-SERVER-KILL-SWITCH VALIDATED / CLOSED — commit c6e5f53; server-side kill switch PLATFORM_CREDITS_RECHARGE_ENABLED gating all four payment/recharge/checkout creation routes; gate fails closed by default (env absent or not exactly 'true' → HTTP 503 PLATFORM_CREDITS_RECHARGE_DISABLED); gated endpoints: POST /api/payment/create + POST /api/payment/china/create + POST /api/credits/manual-recharge + POST /api/billing/checkout; gate placed before any DB write, mock PaymentIntent creation, clientSecret return, China payment provider call, manual recharge order creation; billing/checkout retains original Bearer token auth (gate runs first); paymentLaunchGate.ts pure helper + 14/14 unit tests PASS (node:test); type-check PASS; lint PASS (warnings pre-existing only); build PASS; boundary CLEAN: canvas/generate/provider-adapter/billing-transaction/wallet/ledger/schema/cn-executor/package/lockfile/next.config/Production-DB all zero diff; Production QA PASS: all 4 endpoints return 503 + errorCode PLATFORM_CREDITS_RECHARGE_DISABLED on creator-city-vert.vercel.app; no PaymentOrder created; no clientSecret returned; no Provider called; NEXT: P0-PAYMENT-CREDIT-INTEGRITY-HARDENING-B2 PUSHED_FOR_REVIEW_ONLY / BLOCKED_BY_NO_READONLY_DATABASE)

Last updated: 2026-06-26 (P0-CANVAS-TASK-INPUT-MODE-MATRIX-V1 VALIDATED_FOR_UI_AND_CONTEXT / PROVIDER_REFERENCE_PENDING_FOR_IMAGE — commit ab56d0c; video task dialog "文生视频 / 未连接参考图 / 选择参考图" bar deleted; refImagePickerOpen state + picker modal removed; text-to-video shows clean prompt area; image-to-video (upstream edge): FULLY_CONNECTED unchanged; image-to-video (local ref): NOW FULLY_CONNECTED — local ref mediaUrl flows into video imageUrl → job.input.imageUrl → Seedance image_url content part; image-to-image: SAVED_CONTEXT_ONLY — Seedream does not accept referenceImages; CURRENT_PROVIDER_CAPABILITIES{supportsReferenceImage:false, supportsImageToVideo:true} documents real state; getTaskInputMode() pure function + 29 tests PASS; UI honestly shows "参考图已保存到任务上下文；当前 Provider 不支持直接参考图输入" for image nodes; type-check PASS; boundary CLEAN: generate/image, generate/video, upload, prisma, cn-executor, billing, providers, package, lockfile, next.config all zero diff; NEXT: P0-CANVAS-REFERENCE-IMAGE-TO-PROVIDER-CONTEXT)

Last updated: 2026-06-25 (P0-CANVAS-DIALOG-LOCAL-REFERENCE-UPLOAD-V1 VALIDATED / CLOSED — commits 157695d (impl) + docs; Text upload FULLY_CONNECTED: upload .txt/.md/.markdown/.fountain/.csv/.srt/.vtt/.json → preview card → "应用到 Prompt" → canvasPrompt updated → node.prompt patched → prompt field in API payload; no Provider called during upload or apply; Image reference SAVED_CONTEXT_ONLY: upload JPG → /api/assets/upload → assetId+mediaUrl in node.metadataJson.taskInputs.references; NOT forwarded to Provider (inputAssets:[] hardcoded for image/video; Volcengine ARK API body has no referenceImages field); UI honestly discloses: "参考图已保存到任务上下文；当前 Provider 不支持直接参考图输入"; localScriptImport validator: 8 extensions allowed, blocked MIME list (image/video/audio/pdf/doc/rtf/zip), Word/PDF get actionable error; 32/32 unit tests PASS; type-check PASS; boundary CLEAN: generate/upload-server/prisma/cn-executor/billing/providers/package/lockfile/next.config all zero diff; NEXT: P0-CANVAS-REFERENCE-IMAGE-TO-PROVIDER-CONTEXT)

Last updated: 2026-06-24 (P0-CANVAS-LOCAL-IMAGE-IMPORT-V1 VALIDATED / CLOSED — commit 898f518; local jpg/png/webp drag-drop to canvas viewport; files validated (type + 20MB + 8192px dimensions); uploading node created (status=running); /api/assets/upload POST; node patched with stable CDN URL + assetId + metadata (importedFromLocal/importSource/originalFileName/mimeType/uploadedAt); unsupported type shows toast "仅支持 JPG / PNG / WebP 图片"; >20MB shows toast "图片不能超过 20MB"; mixed batch: valid files upload, invalid files show toast; 6/6 P1 UX browser QA PASS; 19/19 unit tests PASS; type-check PASS; build PASS; /api/assets/upload zero diff; schema/cn-executor/generate/billing/env zero diff; production QA (reload/assets library) pending Vercel deploy)
...
Last updated: 2026-06-21 (P0-ASSET-REMIX-PHASE-1A-ARTIFACT-VERIFICATION-AND-LOCAL-CONTRACT-QA COMPLETE / LOCAL_ONLY — Track 0: ignoreDuringBuilds removed, real lint errors fixed, build PASS; Track B/C: SAM 2.1 Hiera Base+ downloaded to artifacts/cache/ (323,606,802 bytes), SHA-256=a2345aede8715ab1d5d31b4a509fb160c5a4af1970f199d9054ccfb746c004c5 VERIFIED (streaming + shasum double-check); Track D: manifest updated locally — approvalStatus=FOUNDER_APPROVAL_PENDING / verificationStatus=HASH_VERIFIED / sha256=VERIFIED / NOT APPROVED; Track E: artifact security tests written (13 tests — ArtifactRegistry class, ArtifactStatus enum, cache path traversal, gitignore coverage, approved count = 0 guard); Track F: full pipeline contract tests written (14 tests — all state transitions, RunPod ID never in response, dedup, repeated poll no re-ingest); Track G: HMAC/replay integration tests written (25 tests — tampered body, stale timestamp, future timestamp, wrong secret, missing headers, nonce replay, same-body-different-nonce, idempotency, cross-project 404, constant-time compare); Track H: OSS contract tests written (17 tests — key prefix, path traversal, ctid format, PUT URL not in response, signed URL not hardcoded in observability); Track I: ingest contract tests written (13 tests — payload fields, HMAC token not leaked, exactly-once semantics, done fields stable); Track J: cancel/expiry tests written (20 tests — cancel queued/running, RunPod cancel called, idempotency, expiry can_retry, terminal state skips RunPod repoll, error mapping); Track K: pytest 114/114 PASS (0 fail); Track L: security grep CLEAN (no runsync, no DATABASE_URL, no Prisma, no hardcoded keys, no signed URL leaks, RunPod ID logged as 8-char prefix only); Track M: pnpm type-check PASS · lint PASS (pre-existing warnings only) · generate routes untouched · _ARTIFACT_APPROVED=False confirmed · all feature gates closed; gateway/app/main.py: AssetTransformError exception handler added (400 response); NO COMMIT / NO PUSH / NO APPROVE; Artifact status: FOUNDER_APPROVAL_PENDING — awaiting Founder sign-off before APPROVED)

Last updated: 2026-06-21 (P0-ASSET-REMIX-PHASE-1A-COMMIT-AND-LOCAL-BASELINE-CLOSEOUT VALIDATED / CLOSED — Creator City commit 7fb75aa pushed to main (7 files: route.ts Option B rewrite + aliyun-oss.ts presigned PUT fix + assetTransformOss.ts + assetTransformHmac.ts + gatewayServiceClient.ts + ingest/route.ts + next.config.js eslint.ignoreDuringBuilds); type-check PASS · build PASS · diff-check PASS; Gate Matrix: ASSET_TRANSFORM_ENABLED + ASSET_TRANSFORM_GATEWAY_URL=feature active; ASSET_TRANSFORM_OUTPUT_INGESTION_READY=POST accepts submissions; ASSET_TRANSFORM_INTERNAL_INGEST_ENABLED=ingest active (NOT SET); sub-statuses: Architecture=OPTION_B/DESIGN_FROZEN · Creator City Gateway=COMMITTED/PRODUCTION_HIDDEN · OSS Signed GET/PUT=FOUNDATION_COMMITTED · Internal Ingest=CONTRACT_COMMITTED/PRODUCTION_DISABLED · Executor Scaffold=LOCAL_COMMITTED/NO_REMOTE (0c03370, 52 files, 23/23 tests PASS) · Redis Job State=IMPLEMENTED_IN_SCAFFOLD · RunPod Worker=SCAFFOLD_ONLY · SAM Artifact=CANDIDATE/SHA256_PENDING · Approved Artifacts=0 · Production Tools=HIDDEN; Vercel deployment triggered by 7fb75aa; NEXT: P0-ASSET-REMIX-PHASE-1A-ARTIFACT-VERIFICATION-AND-LOCAL-CONTRACT-QA)

Last updated: 2026-06-21 (P0-ASSET-REMIX-PHASE-1A-ARCHITECTURE-EXECUTOR-OSS-CLOSEOUT COMPLETE / AWAITING_FOUNDER_COMMIT_AUTHORIZATION — 12-track closeout: Track A (6 design docs Revision 3) committed 8d8f4c4; executor scaffold 51 files in /Users/aaron/creator-city-asset-executor/ (NOT committed — separate repo); CC code changes UNCOMMITTED / AWAITING_FOUNDER: route.ts (Option B rewrite), aliyun-oss.ts (presigned PUT fix), NEW: assetTransformOss.ts + assetTransformHmac.ts + gatewayServiceClient.ts + /api/internal/asset-transform/ingest/route.ts; type-check PASS; pytest 23/23 PASS; compileall CLEAN; docker compose valid; NO secrets hardcoded; ADR created at docs/ADR_ASSET_REMIX_EXECUTOR_ARCHITECTURE.md; Validation: path-traversal guard added to gateway/app/validation.py; getCapabilities() export added to gatewayServiceClient.ts; capabilities probe simplified in route.ts; ingest route OUTPUT_INGESTION_IMPLEMENTATION_PENDING (returns 503 until Founder enables); open pre-conditions: SAM2 SHA-256 verify + RunPod account + Railway project + 3 CC env vars set + Founder commit authorization for CC code + executor repo init)

Last updated: 2026-06-21 (P0-ASSET-REMIX-EXECUTOR-PHASE-1A-SUBJECT-EXTRACTION-DESIGN APPROVED / DESIGN_FROZEN — Revision 3; Founder confirmed Option B: Independent Gateway Service on Railway Pro + Railway Redis; 6 design docs committed to main; Architecture: CC Gateway generates presigned GET+PUT URLs → Gateway Service (Railway) calls RunPod → worker PUTs to OSS → Gateway validates (HEAD+SHA-256) → Gateway calls CC Internal Ingest API → Asset created; Gateway holds RUNPOD_API_KEY + CC_INTERNAL_INGEST_TOKEN; CC holds OSS creds + DB; no Prisma in Gateway; ~275 lines CC change (no schema migration); 3 new CC env vars: ASSET_TRANSFORM_GATEWAY_URL / ASSET_TRANSFORM_GATEWAY_SERVICE_TOKEN / CC_INTERNAL_INGEST_TOKEN; SAM 2.1 Apache-2.0 CANDIDATE/SHA256_PENDING; executor repo: creator-city-remix-executor; scaffold NOT committed; NO infra created; NO env vars set; open pre-conditions: SAM2 SHA-256 verify + RunPod account + Railway project + CC env vars; Section 15 Founder Decision Record added to ASSET_REMIX_EXECUTOR_PHASE1A_DESIGN.md)

Last updated: 2026-06-21 (P0-ASSET-REMIX-EXECUTOR-PHASE-1A-SUBJECT-EXTRACTION-DESIGN DESIGN_COMPLETE / WAITING_FOUNDER_REVIEW — Revision 2; Founder-directed revision: Option A (direct RunPod adapter inside CC Gateway, no independent Executor Gateway service) + presigned PUT output (worker PUTs directly to CC OSS); 6 design docs revised; Key changes vs Revision 1: architecture → Option A; output transmission → presigned PUT (not ephemeral re-download); HMAC token now carries rpjid (RunPod job ID) embedded after submit — no state table needed; 9-status state machine (queued/running/uploading/ingesting/done/failed/cancelled/expired/needs_user_selection); new error codes: ARTIFACT_HASH_MISMATCH / RUNPOD_UNAVAILABLE / TRANSFORM_TOKEN_INVALID / TRANSFORM_TOKEN_EXPIRED / USER_SELECTION_REQUIRED; executor repo name: creator-city-remix-executor; new modules: artifact_registry.py + observability.py; artifact_manifest.json format defined (YAML + JSON both specified); OSS presigned PUT gap identified: getAliyunOssSignedUploadUrl is a stub (aliyun-oss.ts:326) — ~15-line fix required; MEDIUM confidence band added to subject selection (needs_user_selection → Phase 1A returns USER_SELECTION_REQUIRED as failure; Phase 1B: candidate mask picker); 不得默认只选最大区域 rule added; reload recovery via CanvasWorkflow.metadataJson.pendingAssetTransforms (max 5 writes/job, existing merge mechanism at canvas PUT route:415); minimum CC code scope: ~385 lines, no schema migration, key new files: transformToken.ts / runpodAdapter.ts / assetTransformGateway.ts; uploading_ms timing field added to benchmark plan; 7 open decisions for Founder confirmation; NO code changed; NO commit; NO deploy; docs: ASSET_REMIX_EXECUTOR_PHASE1A_DESIGN.md · ASSET_REMIX_RUNPOD_ADAPTER_CONTRACT.md · ASSET_REMIX_OUTPUT_INGESTION_DESIGN.md · ASSET_REMIX_SUBJECT_SELECTION_STANDARD.md · ASSET_REMIX_SAM21_ARTIFACT_DRAFT.md · ASSET_REMIX_PHASE1A_BENCHMARK_PLAN.md)

Last updated: 2026-06-21 (P0-ASSET-REMIX-KIT-PHASE-1 FOUNDATION_COMMITTED / BLOCKED_BY_REAL_EXECUTOR — commit pushed to main; sub-status: Contract DONE · Registry DONE (主体抠图/高清重建) · Panels DONE / PRODUCTION_HIDDEN · Feature Gate DONE (ASSET_TRANSFORM_ENABLED + ASSET_TRANSFORM_EXECUTOR_URL + ASSET_TRANSFORM_OUTPUT_INGESTION_READY required) · Capability Discovery DONE (GET /api/asset-transform; TTL cache; all disabled when ingestion not ready) · Gateway Security HARDENED (SSRF removed; ownership verification DB; server-resolves URL; domain allowlist; param allowlist) · Async Contract DONE (POST → /submit only; GET → /status/{id}; bounded polling) · Sync Fallback REMOVED (/transform fallback deleted from route.ts) · Output Ingestion BLOCKED / HARD-GATED (ASSET_TRANSFORM_OUTPUT_INGESTION_READY gate; executor URL never → createNode; outputAssetId required in both panels + VCW defense-in-depth) · Executor NOT DEPLOYED · SAM2 Artifact CANDIDATE / SHA256_PENDING · SwinIR Artifact CANDIDATE / LICENSE_AND_SHA256_PENDING · Real-ESRGAN all checkpoints PRODUCTION_BLOCKED_UNTIL_WEIGHT_LICENSE_VERIFIED · InstantID BLOCKED_PENDING_COMMERCIAL_LICENSE_AND_FACE_COMPLIANCE_REVIEW · Production Tools HIDDEN · RunPod Serverless approved as Phase 1 Alpha Executor platform · 24GB GPU / min=0 / max=3 / concurrency=1 approved)

Last updated: 2026-06-21 (P0-AI-DIRECTOR-PROFESSIONAL-JUDGMENT-SPEC DESIGN_COMPLETE / AWAITING_FOUNDER_REVIEW — docs-only; AI_DIRECTOR_PROFESSIONAL_STANDARD.md + AI_DIRECTOR_V1_ARCHITECTURE.md + AI_DIRECTOR_UI_BLUEPRINT.md created; 12-dimension judgment matrix; 5 judgment types; DirectorRecommendation schema; 6 good examples + 3 bad examples; V1 deterministic checks; 8-section UI blueprint; data availability matrix; NO code/commit/push)

Last updated: 2026-06-21 (P1-NEW-TOOL-SHOT-SEQUENCER-CLOUD-PERSISTENCE IMPLEMENTED / WAITING_BROWSER_QA — commit 3adb201; canvasWorkflow.metadataJson.shotSequence is now cloud source of truth; PUT /api/projects/[id]/canvas accepts optional workflowMetadata field, merges into workflow.metadataJson (no schema change, no new API route); VCW extracts cloud sequence on both /ensure and /canvas load paths; handleSaveSequenceToCloud sends full canvas snapshot + workflowMetadata:{shotSequence} in one PUT; ShotSequencerPanel load priority: localStorage draft > cloud > legacy migration > empty; conflict detection banner when draft ≠ cloud; auto draft-save on every change; migration banner for old creator-city:shot-sequence:{pid} key; cancel clears draft; cloud save failure keeps draft + shows error; 4 files changed; type-check PASS · lint PASS · build PASS · pushed; cross-browser QA required — must verify clean-context reload restores sequence) ‖ P1-NEW-TOOL-SHOT-SEQUENCER-MVP VALIDATED / CLOSED — LOCAL-ONLY MVP (browser QA 26 PASS / 0 FAIL / 3 HARNESS_LIMITATION / 1 NOT_IMPLEMENTED; QA commit cd6abd7; all functional requirements PASS; 3 HARNESS_LIMITATION: drag/locate/remove group-hover buttons; 1 NOT_IMPLEMENTED: cloud persistence → now resolved in 3adb201)

Last updated: 2026-06-21 (P1-NEW-TOOL-SHOT-SEQUENCER-MVP IMPLEMENTED / WAITING_BROWSER_QA — commit cd6abd7; new 镜头编排器 / SHOT SEQUENCER director tool; 5 files: lib/canvas/shot-sequence.ts (ShotSequenceItem/ShotSequenceState types + getShotSequence/saveShotSequence/clearShotSequence/formatSequenceDuration localStorage helpers); ShotSequencerPanel.tsx (DirectorToolPanelFrame indigo accent; sequence view with HTML5 drag-to-reorder; picker view; 从分镜表同步 syncs 镜头·* nodes; 保存顺序 persists to creator-city:shot-sequence:{projectId}; 0 Provider calls; no DB/schema/executor changes); canvasModalTypes.ts (+shot-sequencer); CanvasToolDock.tsx (director menu entry + prop type); VisualCanvasWorkspace.tsx (import + state + reset + switch + callback + render); type-check PASS · lint PASS · build PASS · pushed; QA script: qa_shot_sequencer_mvp.py)

Last updated: 2026-06-21 (NEXT-TASK-QUEUE-RECONCILIATION — P0-DERIVED-EDGE-LABEL-CLOUD-PERSISTENCE is NOT stale: CURRENT_STATUS shows IMPLEMENTED/WAITING_BROWSER_QA since commit 6db7f06; browser QA has never been completed; task stays NEXT; no closed items removed; Phase 4 audit added as NEXT implementation task; Complete Timeline remains NOT STARTED; 11 genuine pending P0 browser QA tasks identified — all correctly marked; no regression evidence for any validated feature)
