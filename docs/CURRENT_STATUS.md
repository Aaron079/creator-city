...
Last updated: 2026-08-04 (P1-CANVAS-KEYFRAME-EXTRACTOR-PROVENANCE HARDENING IMPLEMENTED / PREVIEW_HTTP_PERSISTENCE_QA_PASS / BROWSER_HARNESS_LIMITATION -- commits `631bfad`, `87e422b`, and `2b98851` add a versioned local-only keyframe provenance record to explicit image/video draft requests. Evidence is fail-closed: a browser-frame preview requires a local frame and `available` status; timestamp-only references remain truthful for CORS/video failures. Draft nodes carry `metadataJson.keyframeExtraction` and a `关键帧参考` edge identity while the source remains immutable. Focused contract/component tests pass 45/45; web type-check and web build pass; root type-check/build remain blocked by pre-existing server Prisma generated-client errors. An isolated Preview integration registered a disposable QA account, created a project, saved one source video plus image/video keyframe drafts, then reloaded 3 nodes and 2 keyframe edges with versioned timestamp-only provenance intact; no Provider, generation, payment, billing, schema, environment, cn-executor, or Production DB action occurred. The Chrome harness can no longer read or screenshot the large authenticated Canvas DOM without timing out, so UI click-path and network-console capture remain explicitly unclaimed.)
