...
Last updated: 2026-08-03 (P1-CANVAS-KEYFRAME-EXTRACTOR-PROVENANCE HARDENING IMPLEMENTED / PREVIEW_QA_LIMITATION -- commits `631bfad`, `87e422b`, and `2b98851` add a versioned local-only keyframe provenance record to explicit image/video draft requests. Evidence is fail-closed: a browser-frame preview requires a local frame and `available` status; timestamp-only references remain truthful for CORS/video failures. Draft nodes carry `metadataJson.keyframeExtraction` and a `关键帧参考` edge identity while the source remains immutable. Focused contract/component tests pass 45/45; web type-check and web build pass; root type-check/build remain blocked by pre-existing server Prisma generated-client errors. No Provider, generation, payment, billing, schema, environment, cn-executor, or Production DB action occurred. Preview save/reload/network QA requires a disposable video node and is not claimed as passed.)
