...
Last valid commit: Web3 Foundation P1-3 IMPLEMENTED — Marketplace Admin Console（新增 /admin/marketplace 市场运营管理台，退款申请 tab 支持 PENDING/APPROVED/REJECTED/EXECUTED/EXECUTION_FAILED/CANCELLED 状态查看，approve/reject/execute/approveAndExecute 操作含二次确认，EXECUTION_FAILED 显示 executionNote，APPROVED 显示"已批准待执行"提示，订单概览 tab 只读显示全平台 MarketplaceOrder，GET /api/admin/marketplace/orders 新增只读 API，/admin 首页增加"市场管理"入口卡片含 PENDING 退款数量 alertBadge；不改 schema，不改 refund execution 事务，不新增账务行为，P1-2 仍 QA_DEFERRED_TO_UNIFIED_CODEX_RUN）
