...
Last updated: 2026-06-29 (P0-CANVAS-STORYBOARD-GRID-SPLIT-V1 IMPLEMENTED / PRODUCTION_UPLOAD_QA_BLOCKED — commits 1a78671 + e81c1ca + a4c9506; implemented image-node storyboard grid split via asset menu and node tool registry; V1 layouts: 1x2, 2x1, 2x2, 3x2, 2x3, 3x3, 4x3; client Canvas 2D crop helpers + detection/manual fallback; cell upload posts only allowlisted lineage fields to /api/assets/upload when toolId=storyboard-grid-split; no arbitrary metadataJson passthrough; source node immutable with gridCropSession metadata, child image nodes carry cropLineage and edge label; production QA: entry visible under 资产 → 分镜工具 → 分镜拆格; panel opens; proxy source fix lets historical generated images be read by Canvas; local tests PASS: 9/9 storyboard/upload metadata; type-check PASS; lint PASS (pre-existing warnings); build PASS; boundary CLEAN: no generate/provider/billing/payment/schema/cn-executor/package/lockfile/next.config changes; BLOCKER: production crop upload returns “项目验证失败，请重试” from /api/assets/upload project check, even after retry and serialized client uploads; Vercel runtime-log MCP unavailable; final create child nodes/reload/assets recovery QA remains blocked until upload project-check/runtime DB issue is resolved)
