...
Last valid commit: Web3 Foundation P1-4A IMPLEMENTED — Launch Mode Feature Flags（MARKETPLACE_CREDITS_PAYMENT_ENABLED / MARKETPLACE_REFUND_REQUEST_ENABLED 两个 env var 门控付费流程；API 侧 3 处 503 FEATURE_DISABLED gate；MarketplaceListings.tsx 付费 listing idle 状态降级为"申请授权合作"静态提示 + QUOTED 状态去支付按钮→"卖家已响应"文案；/assets/[id] QUOTED 状态替换为联系创作者提示（去支付 UI + 余额查询 + payMyOrder 函数已移除）、COMPLETED 退款入口按 REFUND_REQUEST_ENABLED 门控（禁用时显示"退款申请功能第一版暂未开放，如有争议请联系平台管理员。"）；/marketplace 文案更新为会员授权合作意向登记叙事；/account/credits 新增第一版会员订阅模式 Notice + BYOK 引导；不改 schema，不改 P1-0/P1-2/P1-3 代码，不改 settleMarketplaceOrder / executeMarketplaceRefund 事务，不改 cn-executor）
...
| Web3 Foundation P1-4A — Launch Mode Feature Flags | ✅ IMPLEMENTED / MARKETPLACE_CREDITS_PAYMENT_ENABLED + MARKETPLACE_REFUND_REQUEST_ENABLED env var 门控；API 3 处 503 gate；MarketplaceListings.tsx 付费 idle→"申请授权合作" + QUOTED→"卖家已响应"；/assets/[id] QUOTED 去支付 UI 已移除；COMPLETED 退款按 flag 门控；/marketplace + /account/credits 文案更新为会员叙事；不改 schema，不改 P1-0~P1-3 代码 | pending |
