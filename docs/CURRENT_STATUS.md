...
Last valid commit: Web3 Foundation P1-2 IMPLEMENTED — Admin-approved Refund Execution（migration 20260612000004_add_marketplace_refund_execution，MarketplaceOrderStatus REFUNDED + refundedAt，CreditLedgerType MARKETPLACE_REFUND + MARKETPLACE_SELLER_REVERSAL，MarketplaceRefundRequestStatus EXECUTED + EXECUTION_FAILED + executedAt + executionNote，lib/marketplace/refund.ts executeMarketplaceRefund 原子事务，admin PATCH action:execute/approveAndExecute，buyer balance += priceCredits，seller balance -= sellerAmountCredits，LicenseGrant REVOKED，seller 余额不足→EXECUTION_FAILED 无副作用，幂等保护，不接法币/EconomyWallet/PaymentOrder/NFT/chain）
