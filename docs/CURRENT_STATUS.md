...
| Color Grade Palette — Preview Monitor 修复（filter 可见性 + 浮动可调大小窗口） | 🔄 IMPLEMENTED / browser validation pending | `7a26d8d` |
| Color Grade Palette — 创建调色节点工作流（画布草案节点 + edge + idle 状态） | 🔄 IMPLEMENTED / browser validation pending | TBD |
...
| Color Grade Palette / 调色盘 | 🔄 implemented / browser validation pending（Editing 剪辑分组；基于 DaVinci Resolve Color Page 研究转译；6 个调色预设；Tone Controls / Saturation Controls / S-Curve Intent / HSL Intent / Output Protection；prompt-level grading，不直接改像素；只追加 prompt，不覆盖；已有 [Color Grade Palette] 段落 MVP 跳过；不自动生成；不消耗 credits；不新增 API；不调用 vision AI；应用后需重新生成）|
...
## Canvas Smart Tools Tool 12 — Color Grade Palette / 调色盘 — IMPLEMENTED / browser validation pending

**Status:** 🔄 IMPLEMENTED / browser validation pending
**Date implemented:** 2026-06-07
**Commit:** `a8753b4`
**Rework date:** 2026-06-08
**UX fix date:** 2026-06-08

### 前次浏览器验收未通过原因

- UI 第一眼像参数表+大预设卡片，不像专业 Color Page
- 入口原在左侧 Editing 分组，改为节点顶部"剪辑 / Post Suite"入口

### 当前 Rework 内容（2026-06-08）

- **入口移动**：从左侧 Editing 分组 → image/video 节点顶部 AssetAgentToolbar "剪辑" → 后期套件下拉 → 调色盘
- **左侧 Editing 分组**：已移除，不再存在
- **数据结构升级**：WheelSetting 单位改为 ASC CDL-aligned（temperature/tint/luminance: -1..+1; saturation: 0..4 multiplier, 1=neutral）
- **CurveSetting**：contrastCurve（原 shape）+ rgbBias，值命名语义化
- **QualifierSetting**：简化为 protectSkin + greens + blues + neonAccent
- **TextureSetting**：所有字段改为 number（0-100 for effects，-100..+100 for sharpness/detail）
- **panel 重构**：Clip Strip + Color Page Controls + Intent Monitor + Apply Bar，Tabs: Wheels/Curves/Qualifier/Texture/Output
- **开源审计 v2 结论**：评价标准改为能力优先；当前 Tool 12 仍 prompt-level only；未来路线：Tool 12.5 WebGL LUT preview → Tool 13 FFmpeg worker + LUT3D → 长期 OpenColorIO / AI LUT
- **UX fix (2026-06-08)**：Browser feedback: opening from node top "剪辑 → 调色盘" still felt like user must re-select a target. Fix: NodeTargetBanner added — shows current node thumbnail + "当前调色目标" label immediately below header. Clip Strip demoted to secondary "项目素材 / 切换" role. Opening from a node now auto-selects and visually locks that node as the primary target without requiring any user action in the Clip Strip. Preview Monitor shows image thumbnail or video player with controls. Not pixel grading — prompt-level only.
...
| 模块 | 状态 |
|------|------|
| Preset Strip（6 个调色预设） | ✅ |
| Node Selection（image/video 节点多选） | ✅ |
| Tone Controls（Contrast / Blacks / Whites） | ✅ |
| Saturation Controls（Master Sat / Color Boost / Midtone Detail） | ✅ |
| S-Curve Intent（5 种曲线意图） | ✅ |
| HSL Intent（皮肤保护 / 天空 / 植被 / 色彩强调，prompt-level only） | ✅ |
| Output Protection（始终开启，强制主体保护） | ✅ |
| Prompt Preview（生成调色预览文本） | ✅ |
| Apply Bar（追加到 Prompt / 重置 / 复制报告） | ✅ |

**Phase 2（留 future）：** Primary Wheels 色轮交互 UI / HDR Zones 精确分区 / 真实 Curves 编辑器 / 真实 Qualifier / 真实 Power Windows / 真实 Scopes
...
| # | 步骤 | 预期结果 |
|---|------|---------|
| 1 | 画布左侧工具栏出现 SlidersHorizontal 图标 | ✅ Editing / 剪辑分组图标 |
| 2 | 点击图标 → 展开子菜单 | 出现"调色盘"选项 |
| 3 | 点击"调色盘" | 打开 Color Grade Palette 面板 |
| 4 | 面板标题 | "Color Grade Palette / 调色盘 · 专业影调与色彩控制" |
| 5 | 顶部 amber 横幅 | "Prompt 级调色控制 — 非像素级调色" + "应用后需要重新生成才会看到效果" |
| 6 | 节点选择区 | 显示 image/video 节点，text 节点不显示 |
| 7 | 默认选中 | 当前 active image/video 节点自动选中 |
| 8 | 预设条 | 显示"手动" + 6 个调色预设卡片，带彩色 accent 条 |
| 9 | 预设不含 | 王家卫、Kodak、库布里克等 Look Package 名 |
| 10 | 点击预设 | 各模块参数联动更新 |
| 11 | Tone Controls | Contrast / Blacks / Whites 三段式选择 |
| 12 | Saturation Controls | Master Sat 五档 / Color Boost 四档 / Midtone Detail 四档 |
| 13 | S-Curve Intent | 5 个单选项，带描述 |
| 14 | HSL Intent | 皮肤保护 toggle 默认开 / 天空 / 植被 / 色彩强调选择 |
| 15 | HSL Intent 标注 | "prompt-level only — 非真实像素选区" |
| 16 | Output Protection | 显示"始终开启"绿色标签，强制保护不可关闭 |
| 17 | 点击"生成调色预览" | 显示 [Color Grade Palette] 完整 prompt 文本 |
| 18 | 预览包含各段落 | [Primary / Tone Intent] / [Saturation] / [Curve Intent] / [HSL Intent] / [Output Protection] / [Color Grade Negative Constraints] |
| 19 | 中性参数不输出 | contrast=medium / blacks=neutral 时对应段落跳过 |
| 20 | 点击"追加到 Prompt" | 节点 prompt 末尾追加调色指令 |
| 21 | 原 prompt 保留 | 不覆盖，只追加 |
| 22 | 已有调色段落的节点 | 显示 amber 警告"已跳过" |
| 23 | 应用成功反馈 | "✅ 已追加调色描述 — 请重新生成查看效果" |
| 24 | 复制报告 | 复制调色参数摘要 + 完整 prompt 文本 |
| 25 | 重置调色 | 所有参数回到默认值 |
| 26 | 调色盘在 Editing 分组 | 不在 Prompt / Director / Asset 分组 |
| 27 | 右侧工具栏 | 仍只保留生成前体检，不受影响 |
| 28 | Tool 1-11 | 无回归 |
| 29 | Text/Image/Video 生成 | 无回归 |
| 30 | type-check / build | 通过 ✅ |
