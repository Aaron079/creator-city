...
describe('Creator Skill Engine Stage C canvas boundary', () => {
  ...
  test('all canvas route exits flush Director drafts before snapshots or navigation', () => {
    ...
    assert.match(interactionGate, /SAME_CONTEXT_TARGETS\s*=\s*new\s+Set\(\[[^\]]*['_"]self/)
    ...
    assert.match(interactionGate, /['_"]parent/)
    assert.match(interactionGate, /['_"]top/)
  })

  test('recovery gate portals above every product layer and isolates all body siblings', () => {
    assert.match(interactionGate, /createPortal\(/)
    assert.match(interactionGate, /RECOVERY_Z_INDEX\s*=\s*2147483647/)
    assert.match(interactionGate, /document\.body\.children/)
    assert.match(interactionGate, /new\s+MutationObserver\(/)
    ...
    assert.match(interactionGate, /document\.addEventListener\(['"]pointerdown['"],[\s\S]*true\)/)
    assert.match(interactionGate, /document\.addEventListener\(['"]focusin['"],[\s\S]*true\)/)
    assert.match(interactionGate, /onKeyDownCapture=\{handleRecoveryKeyDownCapture\}/)
    assert.match(interactionGate, /onKeyUpCapture=\{handleRecoveryKeyUpCapture\}/)
    ...
    assert.match(interactionGate, /flex:\s*['"]1 1 0%['"]/)
    assert.match(interactionGate, /height:\s*['"]100%['"]/)
  })
